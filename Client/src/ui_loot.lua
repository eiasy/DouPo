...
local function btn_compoundAnimation()
    ...
    local function callbackFunc()
      UIManager.showToast("恭喜您获得【" .. dictName .. "】" .. "已放入您的背包")
      UILoot.isFlush = true
      UILoot.setup()
...
local function setPageViewItem(item, index)
    ...
    ui_times:setString("拼合次数：" .. compoundTimes)
...
function UILoot.updateTime()
    ...
    if UILoot.countDownTime ~= 0 then 
        ...
        if  UILootChoose.Widget then 
            ...
            ccui.Helper:seekNodeByName(UILootChoose.Widget, "text_time_left"):setString(string.format("保护时间剩余：   %02d:%02d:%02d",hour,min,sec))
        end
    else
       UILoot.stopSchedule()
    end
