...
local function setScrollViewItem(item, obj)
    ...
    local function btnEvent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            if tonumber(obj[4]) ~= 3 then
                if not enough then
                    UIManager.showToast("消耗金币数量不足！")
                else
                    sendNetMessage(obj[1], obj[3])
                end
            else
                UIManager.showToast("累计消耗活动已结束！")
            end
        end
    end
    ...
    if tonumber(obj[4]) == 2 then
        titleText:setString("每日消耗" .. obj[2] .. "元宝" .. "(" .. obj[2] .. "/" .. obj[2] .. ")")
        _btn:setTitleText("已领取")
        _btn:setBright(false)
        _btn:setEnabled(false)
    elseif not enough then
        titleText:setString("每日消耗" .. obj[2] .. "元宝" .. "(" .. UIActivityCostAll.todayCost .. "/" .. obj[2] .. ")")
        _btn:setTitleText("领取")
        _btn:setBright(false)
        _btn:setEnabled(false)
    else
        titleText:setString("每日消耗" .. obj[2] .. "元宝" .. "(" .. obj[2] .. "/" .. obj[2] .. ")")
...
function UIActivityCostAll.setup()
    ...
    local function costCallBack(pack)
        ...
        if startTime and endTime then
            ...
            timeTitle:setString(string.format("活动时间：%d月%d日 %02d:%02d:%02d - %d月%d日 %02d:%02d:%02d", startTime[2], startTime[3], startTime[5], startTime[6], startTime[7], endTime[2], endTime[3], endTime[5], endTime[6], endTime[7]))
        end
    end
