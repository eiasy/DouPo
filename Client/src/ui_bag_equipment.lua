...
local function netCallbackFunc(pack)
  ...
  elseif tonumber(pack.header) == StaticMsgRule.equipPiece then
    UIManager.showToast("恭喜您获得" .. CollectEquipName )
    UIManager.flushWidget(UIBagEquipment)
  end
end
...
local function setScrollViewItem(flag,_Item, _obj)
    if flag == 1 then 
        ...
        if equipCardInstId == 0 then 
            ...
            ItemEquipFor:setString("装备于" .. cardName)
        end
        local inlayThingId = {}
...
--                   UIManager.pushScene("ui_equipment_advance")
                ...
                elseif sender == btn_inlay then --镶嵌
                  if net.InstPlayer.int["4"] < 16 then 
                    UIManager.showToast("战队等级达到16级开启！")
                    cc.release(sender)
                    return 
                  end
...
function UIBagEquipment.init()
	...
	local function btnTouchEvent(sender, eventType)
      ...
      AudioEngine.playEffect("sound/button.mp3")
        ...
        local hint =nil
        if equipFlag ==1 then 
            hint = "购买5个装备背包上限，需要花费" .. expandPrice .. "个元宝"
        else
            hint = "购买5个装备碎片背包上限，需要花费" .. expandPrice .. "个元宝"
        end
        utils.PromptDialog(ExpandCallBack,hint)
			elseif sender == btn_sell then
				...
				if next(EquipThing) then 
			     ...
			     if equipFlag == 1 then 
			         UIManager.showToast("没有可以出售的装备")
			     else
			         UIManager.showToast("没有可以出售的装备碎片")
			     end
			  end
			end
		end
...
function UIBagEquipment.setup()
  ...
  if next(EquipThing) then
    ...
    text_ceiling:setString(string.format("背包上限%d/%d",#EquipThing,grid))
    if btn_chip then
