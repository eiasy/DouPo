                ...
                if flag == 1 then --装备
                    for key, _thingId in pairs(inlayThingId) do
                        if _thingId ~= 0 then
                            ...
                            UIManager.showToast("请先将魔核卸下！")
                            return
                        end
                    end
                end
                number = number +1
                text_choose:setString(string.format("已选择：%d/5",number))
                if number > 5 then 
...
--                end
    ...
    if flag == 1 then --装备
        ...
        local smallUiId=dictEquipData.smallUiId
        local smallImage= DictUI[tostring(smallUiId)].fileName
        local qualityId = dictEquipData.equipQualityId
        ...
        ui_level:setString(string.format("%d级",_obj.int["5"]))
        ui_pz:setString(string.format("品质%d",qualityLevel))
    ...
    elseif flag == 2 then  -- 卡牌
        ...
        ui_level:setString(string.format("等级：%s",_obj.int["9"]))
...
function UIResolve_list.init()
    ...
    local function okEvent(sender,eventType)
        if eventType == ccui.TouchEventType.ended then 
           ...
           elseif _operateType == UIResolve.operateType.rinne then
                UIResolve.setRinneData(instPlayerCardObj)
           end
       end
    end
   ...
   scrollView:removeAllChildren()
    ...
    elseif _operateType == UIResolve.operateType.rinne then
         if net.InstPlayerCard  then 
           for key, obj in pairs(net.InstPlayerCard) do
              if obj.int["10"] == 0 and obj.int["9"] >1 and obj.int["15"] == 0 and obj.int["4"] > StaticQuality.green then ---未上阵 大于1级 不上锁的
                 table.insert(resolveThing,obj)
              end
           end
           utils.quickSort(resolveThing,compareCard)
         ...
         addFlag = 2
         ccui.Helper:seekNodeByName(UIResolve_list.Widget,"btn_equipment"):setVisible(false)
