...
local function netCallbackFunc(data)
	...
	if _skillOpenName then
		local skillLabel = ccui.Text:create("解锁了新技能", dp.FONT, 35)
...
function UICardAdvance.setup()
	...
	if net.InstPlayerCard and _instCardId then
		...
		if qualityId > 0 then
			...
			UICardAdvance.Widget:getChildByName("image_basemap"):getChildByName("image_lv"):getChildByName("text_lv"):setString("等级：" .. cardLevel)
			...
			_cardImagePath = "image/" .. DictUI[tostring(dictCardData.bigUiId)].fileName
			...
			if _newDictData then
				...
				ui_newCardFrameBg:getChildByName("image_warrior"):loadTexture("image/" .. DictUI[tostring(dictCardData.bigUiId)].fileName)
				...
				ui_cardPropPanel:getChildByName("text_blood"):setString("生命 +" .. tonumber(image_arrow1:getChildByName("text_blood_after"):getString()) - tonumber(image_arrow1:getChildByName("text_blood_before"):getString()))
				ui_cardPropPanel:getChildByName("text_attack_gas"):setString("物攻 +" .. tonumber(image_arrow2:getChildByName("text_attack_gas_after"):getString()) - tonumber(image_arrow2:getChildByName("text_attack_gas_before"):getString()))
				ui_cardPropPanel:getChildByName("text_defense_gas"):setString("物防 +" .. tonumber(image_arrow3:getChildByName("text_defense_gas_after"):getString()) - tonumber(image_arrow3:getChildByName("text_defense_gas_before"):getString()))
				ui_cardPropPanel:getChildByName("text_attack_soul"):setString("法攻 +" .. tonumber(image_arrow4:getChildByName("text_attack_soul_after"):getString()) - tonumber(image_arrow4:getChildByName("text_attack_soul_before"):getString()))
				ui_cardPropPanel:getChildByName("text_defense_soul"):setString("法防 +" .. tonumber(image_arrow5:getChildByName("text_defense_soul_after"):getString()) - tonumber(image_arrow5:getChildByName("text_defense_soul_before"):getString()))
				...
				for key, obj in pairs(conds) do
					...
					if tonumber(data[1]) == 1 then --卡片等级_此卡牌等级
                        _tempListData.condsDesc = _tempListData.condsDesc .. "英雄等级达到：LV." .. data[2]
					...
					elseif tonumber(data[1]) == 2 then --卡牌开启第几个命宫_
						...
						for k_, id in pairs(instConstellId_table) do
							if k_ == tonumber(data[2]) then
                                ...
                                _tempListData.condsDesc = _tempListData.condsDesc .. "开启命宫：" .. DictConstell[tostring(instConstellData.int["4"])].name
					...
					elseif tonumber(data[1]) == 3 then --有一张或多张指定的卡牌_(品质Id_星级Id_张数)
                        ...
                        _tempListData.condsDesc = _tempListData.condsDesc .. "拥有卡牌：" .. dictCardData.name .. "（" .. _dictQualityData.name .. _dictStarLevelData.name .. "）X" .. data[4]
...
--						end
					...
					elseif tonumber(data[1]) == 4 then --卡牌称号达到什么阶段_详细称号字典表的Id
                        _tempListData.condsDesc = _tempListData.condsDesc .. "境界达到：" .. DictTitleDetail[data[2]].description
					...
					elseif tonumber(data[1]) == 5 then --升级材料_(物品Id_材料个数)
                        ...
                        _tempListData.condsDesc = _tempListData.condsDesc .. "所需材料：" .. DictThing[tostring(_thingId)].name .. " " .. _haveThingNums .. "/" .. _thingNum
						if _haveThingNums >= _thingNum then --已达标
                            _tempListData.isJude = 1
						end
                    elseif tonumber(data[1]) == 6 then --吞噬异火(_异火ID)
                        local _dictFireId = tonumber(data[2])
                        _tempListData.condsDesc = _tempListData.condsDesc .. "进阶需吞噬异火：" .. DictYFire[tostring(_dictFireId)].name
				...
				ui_cardPropPanel:getChildByName("text_blood"):setString("生命 +0")
				ui_cardPropPanel:getChildByName("text_attack_gas"):setString("物攻 +0")
				ui_cardPropPanel:getChildByName("text_defense_gas"):setString("物防 +0")
				ui_cardPropPanel:getChildByName("text_attack_soul"):setString("法攻 +0")
				ui_cardPropPanel:getChildByName("text_defense_soul"):setString("法防 +0")
			end
		else
			cclog("---------------ERROR:该卡牌不能进阶---------------")
		end
