...
local function netCallbackFunc(pack)
    ...
    UIManager.showToast("恭喜您获得".. number ..  "个".. upgradedObj.name .."!")
    number = 0
          ...
          elseif sender == btn_switch then 
            ...
            if number ~= 0 then 
                if DictThing[tostring(obj.int["3"])].level == MAXUPGRADELEVEL then
                    UIManager.showToast("已达最高等级！")
                    return
                end
                 if number*3 > obj.int["5"] then 
                    UIManager.showToast("魔核个数不足，无法升级！")
                    return
                 end 
                 if cost_money <= net.InstPlayer.int["5"] then 
                    ...
                    UIManager.showToast("您的银币不足，无法升级！")
                 end
             else
                 UIManager.showToast("请选择要升级的数量！") 
             end
...
function UIGemUpGrade.setup()  
    ...
    if obj then 
        ...
        elseif DictThing[tostring(obj.int["3"])].level == MAXUPGRADELEVEL then
            ui_after_name:setString("已达最高等级")
        ...
        ui_before_info:setString("请添加魔核")
        ui_costGem_number:setString("无")
