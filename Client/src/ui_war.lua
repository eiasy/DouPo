UIWar = {
    BATTLE_FIELD_NAMES = { "九龙祭坛", "楠迦城", "天焚妖塔", "蛮荒域" },
    STATE_NONE = 0,
    STATE_EMBATTLE = 1,
    STATE_BATTLE = 2,
    STATE_FINISH = 3,
...
local function setScrollViewItem(item, data)
    ...
    if data[1] == ui.REPORT_TYPE.AMBUSH then
        ...
        text_info:pushBackElement(buildText(ui.commonColor, string.format(" 加入了 %s", ui.BATTLE_FIELD_NAMES[report.state])))
    elseif data[1] == ui.REPORT_TYPE.BATTLEFIELD_DRAW then
        btn_look:hide()
        local fieldId = data[2]
        text_info:pushBackElement(buildText(ui.commonColor, string.format("%s 双方平局，均不得分", ui.BATTLE_FIELD_NAMES[fieldId])))
    elseif data[1] == ui.REPORT_TYPE.BATTLEFIELD then
        ...
        if battleField.state == ui.BF_STATE.WIN then
            ...
            text_info:pushBackElement(buildText(ui.commonColor, string.format(" 占领了 %s 获取 ", ui.BATTLE_FIELD_NAMES[fieldId])))
            text_info:pushBackElement(buildText(color, tostring(ui.battlefieldIntros[fieldId].point)))
            text_info:pushBackElement(buildText(ui.commonColor, " 积分"))
            text_info:pushBackElement(buildBlank(size))

            text_info:pushBackElement(buildText(ui.commonColor, "击杀数："))
            text_info:pushBackElement(buildText(color, tostring(battleField.killCount)))
            text_info:pushBackElement(buildText(ui.commonColor, " 人 获取 "))
            text_info:pushBackElement(buildText(color, tostring(battleField.killPoint)))
            text_info:pushBackElement(buildText(ui.commonColor, " 积分"))
            text_info:pushBackElement(buildBlank(size))

            text_info:pushBackElement(buildText(ui.commonColor, "存活数："))
        ...
        elseif battleField.state == ui.BF_STATE.FAIL then
            text_info:pushBackElement(buildText(color, string.format("【%s】", report.name)))
            text_info:pushBackElement(buildText(ui.commonColor, string.format(" 争夺 %s 失败", ui.BATTLE_FIELD_NAMES[fieldId])))
            text_info:pushBackElement(buildBlank(size))
    ...
    elseif data[1] == ui.REPORT_TYPE.FIGHT then
        ...
        btn_look:unregisterScriptHandler()
        btn_look:addTouchEventListener( function(sender, eventType)
            if eventType == ccui.TouchEventType.ended then
                ...
                netSendPackage( { header = StaticMsgRule.unionReplay, msgdata = { int = { instVideoId = report[4] } } }, function(pack)
                    ...
                    utils.recursionTab(Fight_INIT_DATA, "Fight_INIT_DATA")
                    ...
                    UIFightMain.setData(Fight_INIT_DATA, nil, dp.FightType.FIGHT_UNION_REPLAY, function(isWin)
                        ...
                        local info = "·%s 第%d场 【%s】%s 击败了 【%s】%s"
                        info = string.format(info, ui.BATTLE_FIELD_NAMES[report[2].state], report[1], report[2].unionName, report[2].name, report[3].unionName, report[3].name)
        ...
        text_info:pushBackElement(buildText(ui.commonColor, string.format("·%s 第%d场 ", ui.BATTLE_FIELD_NAMES[report[2].state], report[1])))
        ...
        text_info:pushBackElement(buildText(ui.commonColor, " 击败了 "))
...
local function refreshImage_fight(image_frame, image, fighter)
    ...
    image_frame:unregisterScriptHandler()
    image_frame:addTouchEventListener( function(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            audio.playSound("sound/button.mp3")
            UIManager.showLoading()
            netSendPackage( { header = StaticMsgRule.enemyPlayerInfo, msgdata = { int = { playerId = fighter.id } } }, function(pack)
...
function ui.setup()
    ...
    local text_alliance_name = ccui.Helper:seekNodeByName(UIAlliance.Widget, "text_alliance_name")
    local allianceName = text_alliance_name:getString()
    image_blue:getChildByName("text_name"):setString(allianceName)
    ...
    if ui.state == ui.STATE_NONE then
        ...
        for i, name in ipairs(ui.BATTLE_FIELD_NAMES) do
            ...
            image_look:getChildByName("text_number"):setString("队伍数：0")
            image_seize:hide()
            image_fight:hide()
            image_join:hide()
        end
        ...
        for i, name in ipairs(ui.BATTLE_FIELD_NAMES) do
            ...
            panel_battlefield:unregisterScriptHandler()
            ...
            if ui.state == ui.STATE_EMBATTLE then
                ...
                image_look:show()
                image_look:getChildByName("text_name"):setString(name)
                image_look:getChildByName("text_number"):setString("队伍数：" .. #myBattleField.survive)
                image_seize:hide()
                image_fight:hide()
            elseif myBattleField.state == ui.BF_STATE.WIN or myBattleField.state == ui.BF_STATE.FAIL or myBattleField.state == ui.BF_STATE.DRAW then
                ...
                text_number_blue:setString("队伍数：" .. #myBattleField.survive)
                text_number_red:setString("队伍数：" .. #enemyBattleField.survive)
    ...
    if ui.state == ui.STATE_EMBATTLE or ui.state == ui.STATE_NONE then
        ...
        if ui.lastReportCount ~= #ui.reports then
            ...
            if ui.reports[i][1] == ui.REPORT_TYPE.BATTLEFIELD then
            ...
            while i <= #ui.reports do
                ...
                if report[1] == ui.REPORT_TYPE.FIGHT then
                    report = report[2]
                    str = string.format("实时战报：%s 第%d场 【%s】%s 击败了 【%s】%s", ui.BATTLE_FIELD_NAMES[report[2].state], report[1], report[2].unionName, report[2].name, report[3].unionName, report[3].name)
                elseif report[1] == ui.REPORT_TYPE.AMBUSH then
                    report = report[2]
                    str = string.format("实时战报：【%s】%s 加入了 %s", report.unionName, report.name, ui.BATTLE_FIELD_NAMES[report.state])
                elseif report[1] == ui.REPORT_TYPE.BATTLEFIELD_DRAW then
                    local fieldId = report[2]
                    str = string.format("实时战报：%s 双方平局，均不得分", ui.BATTLE_FIELD_NAMES[fieldId])
                elseif report[1] == ui.REPORT_TYPE.BATTLEFIELD then
                    ...
                    for k = 0, 1 do
                        ...
                        local battleField = report[2].battleFields[fieldId]
                        if battleField.state == ui.BF_STATE.WIN then
                            str = str .. string.format("【%s】 占领了 %s 获取%d积分；击杀数：%d人 获取%d积分；存活数：%d人 获取%d积分", report[2].name, ui.BATTLE_FIELD_NAMES[fieldId], ui.battlefieldIntros[fieldId].point, battleField.killCount, battleField.killPoint, battleField.surviveCount, battleField.survivePoint)
                        else
                            str = str .. string.format(" 【%s】 争夺 %s 失败；击杀数：%d人 获取%d积分；存活数：%d人 获取%d积分", report[2].name, ui.BATTLE_FIELD_NAMES[fieldId], battleField.killCount, battleField.killPoint, battleField.surviveCount, battleField.survivePoint)
                        end
                    end
                    i = i + 1
                end
...
function ui.onWarEvent(msgdata, ignoreAni)
    ignoreAni = ignoreAni and(not ui.Widget or not ui.Widget:getParent())
    ...
    if event == ui.EVENT.INIT then
        ...
        for i = 1, 4 do
            ...
            table.insert(battlefieldIntros, { point = tonumber(battleField.int.score or 0), reward = battleField.string.reward or "" })
        end
        ...
        UIWar.my = ui.myUnionMemberBattle[net.InstPlayer.int["1"]]
        ...
        for i = 1, math.huge do
            ...
            elseif event == ui.EVENT.FIGHT then
                ...
                table.insert(ui.reports, { ui.REPORT_TYPE.FIGHT, { fightId, winner, failer, recordId } })
            end
        end
    ...
    elseif event == ui.EVENT.FINISH then
        local winner = msgdata.int.winer
