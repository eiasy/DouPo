...
local function setScrollViewItem(_item, _data)
	...
	ui_level:setString("等级：".._level)
	ui_fight:setString("战斗力：".._fightVale)
		...
		if eventType == ccui.TouchEventType.ended then
			if sender == btn_yes then
				if userData.currentCount >= userData.totalCount then
					UIManager.showToast("联盟成员人数已达上限！")
				else
...
function UIAllianceApply.setup()
	...
	ui_memberLabel:setString(string.format("成员数：%d/%d", userData.currentCount, userData.totalCount))
end
