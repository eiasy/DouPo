...
local function netCallbackFunc(data)
	...
	if protoCode == StaticMsgRule.train then
        ...
        --]]
		btn_bottomLeft:setTitleText("放弃")
		btn_bottomRight:setTitleText("保留")
		_curMedicineCount = utils.getThingCount(StaticThing.linden)
		ccui.Helper:seekNodeByName(ui_practiceInfo, "text_medicine"):setString("菩提子：" .. _curMedicineCount)
...
local function initPracticeInfo()
	...
	if _param then
		...
		ui_cardIcon:loadTexture("image/" .. DictUI[tostring(dictCardData.bigUiId)].fileName)
		...
		if string.len(dictCardData.nickname) > 0 then
			ui_cardLabel3:setString("资质"..dictCardData.nickname)
...
--		ccui.Helper:seekNodeByName(ui_checkbox2, "text_number"):setString(tostring(_needMedicineCount * 10))--修炼十次
	...
	if instPlayerCardData then
		...
		ccui.Helper:seekNodeByName(ui_practiceInfo, "text_potential"):setString("可分配潜力点：" .. _curPotentialCount)
		_curMedicineCount = utils.getThingCount(StaticThing.linden)
		ccui.Helper:seekNodeByName(ui_practiceInfo, "text_medicine"):setString("菩提子：" .. _curMedicineCount)
...
local function onPracticeEvent()
	if not ui_cardIcon:isVisible() then
		UIManager.showToast("请选择修炼卡牌")
		return
	end

	if _curSelectedTrainPropId == nil or _curTimes == nil then
		UIManager.showToast("请选择修炼项目")
		return
	end
	...
	local function onTouchEvent(sender, eventType)
		if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_help then
                UIAllianceHelp.show({titleName="修炼帮助",type=7})
			end
		end
	end
...
function UICardRealm.setup()
	...
	btn_bottomLeft:setTitleText("退出")
	btn_bottomRight:setTitleText("修炼")
