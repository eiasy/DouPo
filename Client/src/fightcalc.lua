...
function Fight.onFightDone()
	...
	if Fight.initData.isBoss then
		bossDamage = Fight.fighters[BOSS_POSITION]:getHPMax() - Fight.fighters[BOSS_POSITION]:getHP()
...
function Fight.encodeNumber(num)
	...
	local hash = temp:gsub(".",function(c)
			...
			return aaaa[c:byte() - 0x30 + 1] .. aaaa[math.random(1,10)]
		end
	)
	return hash
end
