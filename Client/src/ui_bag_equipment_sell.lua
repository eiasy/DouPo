...
local function netSellFunc(pack)
  UIManager.showToast("恭喜你获得" .. Price .."银币")
  choose_number = 0
  Price=0
  selectedInstId={}
...
local function setChipScrollViewItem(item, data)
	...
	chipDescribe:setString("集齐".. collectNum .. "个碎片可以合成装备")
...
local function setEquipScrollViewItem(item, data)
	...
	equipLevel:setString(string.format("%d级",equipLv))
	...
	local function ui_checkBoxEvent(sender,eventType)
	    ...
	    ui_text_choose_number:setString("已选装备：" .. choose_number)
	    ui_text_sell_number:setString("总计出售：" .. Price)
	  end
