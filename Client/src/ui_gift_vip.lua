...
local function setVipThing(sender)
    ...
    ui_vipTitle:setString(string.format("VIP%d等级特权", dictVip.level))
    ...
    image_base_di:getChildByName("image_title"):getChildByName("text_title"):setString(string.format("VIP %d 礼包", dictVip.level))
    local _state = 0
    -- 0:未领取, 1:领取, 2:已领取
    ...
    for i = 1, 4 do
        ...
        if things[i] then
            ...
            utils.showThingsInfo(thingItem:getChildByName("image_get_good1"), tableData[1], tableData[2])
        end
    end
end
