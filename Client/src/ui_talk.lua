    ...
    if uiView and tempchatInfo then
        for key = #tempchatInfo, 1, -1 do
            ...
            if (index == UITalk.TAG_UNION and string.match(info.contentText, "unionAssist:%d+|%d+_%d+_%d+|%d+|%d+")) == info.contentText then
                ...
                local elements = { }
                table.insert(elements, ccui.RichElementText:create(key, cc.c3b(61, 19, 10), 255, "我已经占领了", dp.FONT, FONTSIZE))
                ...
                table.insert(elements, ccui.RichElementText:create(key, cc.c3b(61, 19, 10), 255, "大家快来协助吧！", dp.FONT, FONTSIZE))
                ...
                local text2 = ccui.Text:create("协助特殊奖励", dp.FONT, FONTSIZE)
                    ...
                    button:addTouchEventListener( function(sender, touchEventType)
                        if touchEventType == ccui.TouchEventType.ended then
                            ...
                            function(pack)
                                ...
                                if type == 1 then
                                    utils.PromptDialog(jumpToMine, string.format("活动时间不足%d小时，将可能无法获得特殊奖励，是否前往？", math.floor(msgdata.int.specialRewardAssistTime / 3600)))
                                elseif type == 2 then
                                    utils.PromptDialog(jumpToMine, "本次活动特殊奖励已被领完，是否前往？")
                                else
                                    jumpToMine(true)
                                end
                            end )
            ...
            headIcon:addTouchEventListener( function(sender, eventType)
                if eventType == ccui.TouchEventType.ended then
                    ...
                    UIAllianceTalk.show( { playerId = _info.roleId, userName = _info.roleName, userLvl = _info.teamLvl, userFight = _info.fight, userUnio = _info.unioName, headId = _info.headId, vip = _info.vipName })
                end
            end
            )
...
local function setBtnTime(time)
    local sec = time % 60
    btn_go:setTitleText(string.format("%02d秒", sec))
end
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_go then
                ...
                if utils.utf8len(text) == 0 then
                    UIManager.showToast("请输入聊天内容")
                elseif utils.utf8len(text) > 50 then
                    UIManager.showToast("聊天内容过长")
                elseif #text > 0 then
