...
function UIArenaHistory.refreshUILabel(_rank, _weiwang)
	if UIArenaHistory.Widget then
		ccui.Helper:seekNodeByName(UIArenaHistory.Widget, "text_up"):setString(string.format("您的排名上升至%d名！", _rank))
		ccui.Helper:seekNodeByName(UIArenaHistory.Widget, "text_get"):setString(string.format("获得%d威望。", _weiwang))
	end
end
