...
local function startFade()
    if _tempLv > _curLevel then
		...
		local maxExp = DictMagicLevel[tostring(_tempLv)].exp
			...
			elseif sender == btn_onekey then
				if net.InstPlayerMagic then
                    ...
                    if isHighQ and #selectedInstMagicIds < 1 then
                        UIManager.showToast("紫色以上物品需手动添加")
                    end
			...
			elseif sender == btn_intensify then
				...
				if _curInstMagicId then
					if _instMagicIds then
						...
						UIManager.showToast("请选择要吞噬的" .. (magicType == dp.MagicType.gongfa and "功法" or "法宝") .. "!")
					end
				end
			end
		end
...
function UIGongfaIntensify.setup()
	if _curInstMagicId then
		...
		ui_titleText:setString((magicType == dp.MagicType.gongfa and "功法" or "法宝") .. "强化")
