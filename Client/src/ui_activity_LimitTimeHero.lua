...
local function countDowun()
    ...
    if UIAactivityLimitTimeHero.Widget then
        ...
        ui_countdownText:setString(string.format("倒计时  %d天%02d:%02d:%02d", day, hour, minute, second))
        ...
        onePanel:getChildByName("text_time"):setString(string.format("%02d:%02d:%02d后免费", _hour, _minute, _second))
    end
end
...
--    local ui_svItem = ui_scrollView:getChildByName("panel"):clone()
    ...
    layoutScrollView(ui_scrollView, ui_svItemRank, _rankListData, function(_item, _data)
        if _data.startRankNum == _data.endRankNum then
            _item:getChildByName("text_rank"):setString(string.format("第%d名", _data.startRankNum))
        else
...
local function initRecruitInfo(_msgData)
    ...
    if _time == 0 then
        btn_one:getChildByName("text_one"):setString("免费\n招募")
        ...
        btn_one:getChildByName("text_one"):setString("黄金\n招募")
        ...
        onePanel:getChildByName("image_jin"):getChildByName("text_cost"):setString(string.format("元宝      %d", _onePrice))
    end
    ...
    ccui.Helper:seekNodeByName(tenPanel, "text_cost"):setString(string.format("元宝     %d", _tenPrice))
    ...
    local function onButtonEvent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            if sender == btn_help then
                UIAllianceHelp.show({titleName="活动详情",type=1})
...
function UIAactivityLimitTimeHero.setup()
    ...
    if DictActivity and DictActivity.string["4"] ~= "" and DictActivity.string["5"] ~= "" then
        ...
        ui_timeText:setString(string.format("活动时间：%d月%d日%d点-%d月%d日%d点", _startTime[2],_startTime[3],_startTime[5],_endTime[2],_endTime[3],_endTime[5]))
