...
local function initAllianceInfo(_msgData)
	...
	ui_loadingLabel:setString("当前建设进度：" .. _msgData.int["12"])
	ui_loadingCount:setString(string.format("今日建设人数：%d/%d", _msgData.int["13"], _msgData.int["7"]))
...
local function initBuildInfo()
	...
	for key, buildItem in pairs(ui_buildItem) do
		...
		buildItem:getChildByName("text_loading"):setString("建设进度+" .. unionBuildData.plan)
		buildItem:getChildByName("text_exp"):setString("联盟经验+" .. unionBuildData.exp)
		buildItem:getChildByName("text_congratulate"):setString("联盟贡献+" .. unionBuildData.contribution)
...
netCallbackFunc = function(msgData)
	...
	elseif code == StaticMsgRule.unionBuild then
		UIManager.showToast("建设成功！")
		UIAllianceBuild.setup()
	elseif code == StaticMsgRule.unionDynamic then
		local _msgContent = "<color=139,69,19>今天还没有人建设</color>"
		if msgData.msgdata.message.unionDynamic and msgData.msgdata.message.unionDynamic.message then
			for key, obj in pairs(msgData.msgdata.message.unionDynamic.message) do
				...
				_msgContent = "<color=139,69,19>成员</color><color=0,128,0>"..obj.string["3"].."</color><color=139,69,19>"..unionBuildData.description.."，增加</color><color=0,0,255>"..unionBuildData.plan.."点</color><color=139,69,19>进度，增加</color><color=0,0,255>"..unionBuildData.exp.."点</color><color=139,69,19>联盟经验</color>"
				break
			end
		end
