...
function UIAllianceTalk.freshInfo( userName , userLvl , userFight , userUnio , headId , vip )
    ui_userName:setString( userName )
    ui_userLvl:setString( "等级："..userLvl )
    ui_userFight:setString( "战斗力："..userFight )
    if userUnio then
        ui_userUnio:setString( "联盟："..userUnio )
    else
        ui_userUnio:setString( "无联盟" )
    end
...
function UIAllianceTalk.show(_tableParams)
    ...
    UIAllianceTalk.freshInfo(userData.userName, userData.userLvl, userData.userFight, userData.userUnio, userData.headId, userData.vip)
end
