...
function showNoEnoughDialog( buyNum )
    ...
    description:setString("银币" .. DictSysConfig[tostring(StaticSysConfig.silverNoteToCopper)].value)
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            if sender == sureBtn then
                if buyNum > 0 then
                    ...
                    UIManager.showToast("今日购买次数已达上限")
                end
        ...
        if scrollUpdate then
            ...
            for tag = top, bottom do
                ...
                if tag == #thingData and _tempSoulId and thingData[ tag ].int[ "3" ] == _tempSoulId then  
                    ...
                    utils.addSoulParticle( image_frame_fire:getParent() , DictFightSoul[ tostring( thingData[tag].int[ "3" ] )].effects )
...
--        end
    ...
    elseif data.header == StaticMsgRule.upgradeFightSoulId then
        ...
        if isHaveDiFightSoul == 1 then
            utils.showDialog( DictFightSoul[ tostring( id ) ].name.."将吞噬其他地阶及以下的斗魂，是否继续？" , sendData1 )
        else
            sendData1()
        end
    end
...
--                addItem( data )
                if net.InstPlayer.int["4"] < 32 then
                    UIManager.showToast( "猎10次需要等级达到32级" )
                else
            ...
            elseif sender == btn_help then
                UIAllianceHelp.show({titleName="斗魂帮助",type=8})
            elseif sender == btn_lighten then --点亮第四个
                if image_card_select[ 4 ] == 1 then
                    UIManager.showToast( "无需重复点亮" )
                else
                   -- setImageCardInfo( 4 ) 
                   sendData( 5 )
                end
            ...
            for i = 1 , 5 do
                if sender == image_card[ i ] then
                    if image_card_select[ i ] == 1 then
                       -- cclog( "猎魂了" )
                        if  tonumber( net.InstPlayer.string["6"] ) < tonumber( DictFightSoulHuntRule[ tostring( i ) ].needSilver ) then
