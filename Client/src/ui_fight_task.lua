...
local  function setScrollViewItem(_Item, obj, _isNewBarrier)
    ...
    local function TouchEvent(sender,eventType)
        if eventType == ccui.TouchEventType.ended then
              ...
              --------------------临时解决方案--------------------
                ...
                if UIGuidePeople.guideStep and UIGuidePeople.guideStep ~= guideInfo["15B5"].step then 
                    ...
                    for key, obj in pairs(DictBarrierLevel) do
                        ...
                        utils.quickSort(TableBarrierLevel,function (obj1,obj2)
                          return obj1.level > obj2.level
                        end)
                    end
                    local param = {}
                    param.barrierLevelId = TableBarrierLevel[1].id
            ...
            if obj.name ~= nil and taskStory ~= nil then ---新关卡才会执行这个
    ...
    for i=1,3 do
        if i <= barrierLevel then
            image_boss_star[i]:loadTexture("ui/star01.png")
        else
            image_boss_star[i]:loadTexture("ui/star02.png")
        end
        if #TableBarrierLevel == 1 then
            image_boss_star[1]:setPosition(cc.p(star1_x+30,star1_y))
            image_boss_star[2]:setVisible(false)
            image_boss_star[3]:setVisible(false)
        end
        if #TableBarrierLevel == 2 then
            image_boss_star[1]:setPosition(cc.p(star1_x+15,star1_y))
            image_boss_star[2]:setPosition(cc.p(star2_x+15,star2_y))
            image_boss_star[3]:setVisible(false)
        end
...
local function TaskGetThingDialog(type)
  ...
  msgLabel:setString("达到".. needStarNum .."★可领取")
  ...
  if getFlag ==  true then 
      sureBtn:setTitleText("已领取")
      utils.GrayWidget(sureBtn,true)
      sureBtn:setEnabled(false)
  else
      sureBtn:setTitleText("领取奖励")
  end
  local guide = nil
...
local function setBoxItem(key,obj)
  ...
  local subPosY = image_basemap:getContentSize().height - pointY ----策划是以图片左上角为原点
    ...
    if not UIFightTask.stopTaskAni then
      if maxBarrierId ~= 0 then
          if maxBarrierLevel ~= 0  then 
            ...
            if lastBarrierId == 0 then ---打完整个副本
              if not basemapPercent then 
                view_basemap:scrollToTop(0.00001,false)
              end
              return
            ...
            if lastBarrier.chapterId == chapterId then ---新关卡
                local _Item = Item:clone()
              ...
              if not basemapPercent then ---打最后新关卡的时候
                view_basemap:scrollToTop(0.00001,false)
              end
            end
          end
      else  ---如果实例表没有数据 就从字典表里寻找最小的关卡id
...
function UIFightTask.showPosterDialog()
    local _isShow = cc.UserDefault:getInstance():getStringForKey("isShowPosterDialog")
    if (_isShow == nil or _isShow == "") and _isShowPosterDialog then
        UIPoster.show()
        cc.UserDefault:getInstance():setStringForKey("isShowPosterDialog", "1")
        _isShowPosterDialog = nil
    end
end

function UIFightTask.setShowPoster(_isShow, _barrierId)
    _isShowPosterDialog = _isShow
    local _isShow = cc.UserDefault:getInstance():getStringForKey("isShowPosterDialog")
    if _isShow == "1" then
        _isShowPosterDialog = nil
    end

    if _isShowPosterDialog then
