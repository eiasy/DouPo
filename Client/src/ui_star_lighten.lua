    ...
    local function onEvent( sender , eventType )
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_help then
                UIAllianceHelp.show( { type = 12 , titleName = "观星帮助" } )
...
function UIStarLighten.setup()
    ...
    local function callBack(pack)
        ...
        bar_star:getChildByName("text_star_number"):setString( "当前星数："..curStarCount )
        local text_get_number = ccui.Helper:seekNodeByName( UIStarLighten.Widget , "text_get_number" )
        text_get_number:setString("今日占卜次数："..tostring(curHoldCount).."/"..DictSysConfig[ tostring(StaticSysConfig.holdStarCanHoldTimes )].value)
        local text_refresh_number = ccui.Helper:seekNodeByName( UIStarLighten.Widget , "text_refresh_number" )
        text_refresh_number:setString("免费刷新次数："..( DictSysConfig[ tostring(StaticSysConfig.holdStarFreeRefreshTimes )].value - curFreeCount))
        ...
        image_ke:getChildByName( "text_ke_number"):setString( DictSysConfig[ tostring(StaticSysConfig.holdStarRefreshThingNum )].value.."个")
        local image_gold = ccui.Helper:seekNodeByName( UIStarLighten.Widget , "image_di" ):getChildByName("image_gold")
        image_gold:getChildByName( "text_gold_number"):setString( "或"..DictSysConfig[ tostring(StaticSysConfig.holdStarRefreshGoldNum )].value.."个")
    end
    callBack()
    local btn_reward = ccui.Helper:seekNodeByName( UIStarLighten.Widget , "btn_reward" )
