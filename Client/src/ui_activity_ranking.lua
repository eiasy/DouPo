...
local function netCallbackFunc(data)
    ...
    if code == StaticMsgRule.ranking then
        ...
        if msgData and #msgData > 0 then
            ...
            local _index = math.floor(#_levelData / PAGE_ITEM_SIZE) * 10 + 1
...
initScrollViewItem = function(flag, item, data)
    if data then
        if flag == TYPE_PILLTOWER then
            ...
            item_playerLevel:setString("等级：" .. data.level)
            if data.allianceName and data.allianceName ~= "" then
                item_allianceName:show():setString("联盟：" .. data.allianceName)
            else
                item_allianceName:hide()
            end
            item_maxPoint:setString(string.format("最高层：%d层", data.maxPoint))
            ...
            if #data.allianceName > 0 then
                text_alliance:show():setString("联盟：" .. data.allianceName)
...
function UIActivityRanking.setup()
    ...
    if _curListType == TYPE_LEVEL then
        image_base_tab:getChildByName("text_lv"):setString("等级：" .. net.InstPlayer.int["4"])
    elseif _curListType == TYPE_ARENA and net.InstPlayerArena then
        image_base_tab:getChildByName("text_lv"):setString("排名：" .. net.InstPlayerArena.int["3"])
    elseif _curListType == TYPE_PILLTOWER and _dantaRank then
        image_base_tab:getChildByName("text_lv"):setString("排名：" .. _dantaRank)
        _dantaRank = nil
    end
    ...
    if _levelData then
        ...
        if #_levelData < MAX_ITEM_SIZE then
