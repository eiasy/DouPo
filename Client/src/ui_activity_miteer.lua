...
local function netCallbackFunc(pack)
    ...
    elseif tonumber(pack.header) == StaticMsgRule.platinumVIP then
        UIActivityMiteer.setup()
        UIActivityMiteer.PromptDialog("恭喜您成为拍卖场白金贵宾，从今天开始米特尔拍卖场将全天为您服务", 1)
    end
end
...
local function updateTime()
    ...
    if ui_timeTextDown:isVisible() then
        if countDownTimeDown ~= 0 then
            ...
            ui_timeTextDown:setString(string.format("上新倒计时  %02d:%02d:%02d", hour, min, sec))
        else
            if ScheduleId then
                cc.Director:getInstance():getScheduler():unscheduleScriptEntry(ScheduleId)
            end
...
local function show(Enable, param)
    if Enable then
        ...
        text_num:setString(string.format("数量：%d", value))
                        ...
                        elseif sender == btn_exchange then
                            ...
                            if obj.int["6"] == 2 then
                                ...
                                if hunyuanNumber < obj.int["7"] then
                                    UIActivityMiteer.PromptDialog("您的魂源不足，分解卡牌可以得到魂源", 2)
                                    return
                                end
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            if sender == btn_vip then
                ...
                if upSilverEnabled == 0 then
                    UIManager.showToast("VIP等级达到" .. openVipNum .. "级开放")
                    return
                end
                sendVipData()
