...
--        initWorldBoss()
    ...
    -- 秒
    -- 时间
    ui_fightPanel:getChildByName("text_time"):setString(string.format("结束倒计时    %02d:%02d:%02d", hour, minute, second))
end
...
local function rebirthCountDown(dt)
    ...
    -- 秒
    _challengeTime:setString(string.format("倒计时：%02d:%02d", minute, second))
end
...
local function countDown(dt)
    ...
    if ui_countdownText then
        ui_countdownText:setString(string.format("倒计时：%02d:%02d:%02d", hour, minute, second))
    end
end
...
local function initPrevRankingData(_uiPanel)
    ccui.Helper:seekNodeByName(_uiPanel, "text_name1"):setString("暂无")
    ccui.Helper:seekNodeByName(_uiPanel, "text_name2"):setString("暂无")
    ccui.Helper:seekNodeByName(_uiPanel, "text_name3"):setString("暂无")
    ccui.Helper:seekNodeByName(_uiPanel, "text_kill"):setString("上轮击杀者：暂无")
    if _playerRankingData then
        local _textIndex = 1
        for key, obj in pairs(_playerRankingData) do
            if obj.int["1"] == 0 then
                ccui.Helper:seekNodeByName(_uiPanel, "text_kill"):setString("上轮击杀者：" .. obj.string["3"])
            else
        ...
        if _boxAnimation then
            ...
            image_box:addTouchEventListener(function(sender, eventType)
                if eventType == ccui.TouchEventType.ended then
                    if _boxState == 0 then
                        UIManager.showToast("您没有参与今日的活动，明天记得来哦！")
                    elseif _boxState == 1 then
...
--                ui_kill:setTag(1)
    ...
    elseif code == StaticMsgRule.joinWorldBoss then
        ...
        if data.msgdata.message and data.msgdata.message.playerMsg then
            ...
            ui_fightPanel:getChildByName("text_number_attack"):setString(string.format("已攻击  %d  次", initData.int["1"]))
            ui_fightPanel:getChildByName("text_hurt_all"):setString(string.format("总攻击伤害：%d", initData.int["2"]))
            ...
            ccui.Helper:seekNodeByName(btn_rebirth, "text_rebirth"):setString("重生 " .._rebirthMoney)
            ui_fightPanel:getChildByName("text_hint_revive"):setString(string.format("还有（%d/3）次重生机会", 3 - initData.int["5"]))
            ...
            ui_fightPanel:getChildByName("text_ranking"):setString("当前排名：" .. initData.int["7"])
            local bossCurBlood = initData.int["8"]
...
function UIBoss.init()
    ...
    ui_countdownText:setString(string.format("倒计时：%02d:%02d:%02d", 0, 0, 0))
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_auto then
                ...
                if net.InstPlayer.int["19"] >= _vipLv then
                    ...
                    if isAutoFight and _rebirthCountDown == 0 and btn_challenge2:getTitleText() == "挑战" then
                        onEnterFight()
                    end
                else
                    UIManager.showToast("自动挑战需要VIP" .. _vipLv)
                end
            elseif sender == btn_rebirth then
                ...
                if _rebirthCountDownId then
                    ...
                    UIManager.showToast("没有冷却时间，无需重生！")
                end
            elseif sender == btn_challenge2 then
                if isCanChallenge then
                    onEnterFight()
                else
                    UIManager.showToast("挑战冷却时间未到！")
                end
            end
        end
    end
    ...
    ccui.Helper:seekNodeByName(ui_normalPanel, "text_name1"):setString("暂无")
    ccui.Helper:seekNodeByName(ui_normalPanel, "text_name2"):setString("暂无")
    ccui.Helper:seekNodeByName(ui_normalPanel, "text_name3"):setString("暂无")
    ccui.Helper:seekNodeByName(ui_normalPanel, "text_kill"):setString("上轮击杀者：暂无")
end
...
function UIBoss.setup()
    ...
    ui_normalPanel:getChildByName("btn_challenge"):setVisible(false)
    ui_normalPanel:getChildByName("image_di"):setVisible(true)
    ui_fightPanel:getChildByName("btn_challenge"):setTitleText("挑战")
...
function UIBoss.pushData(data)
    cclog("--------->>>  世界BOSS同步数据！")
    if data and data.msgdata and data.msgdata.int and data.msgdata.int.worldBossBlood then
        if data.msgdata.int.initWorldBossBlood then
            WORLD_BOSS_TOTAL_BLOOD = data.msgdata.int.initWorldBossBlood
        end
        local bossCurBlood = data.msgdata.int.worldBossBlood
