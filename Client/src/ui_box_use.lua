...
function UIBoxUse.init()
		...
		if eventType == ccui.TouchEventType.ended then
			...
			elseif sender == btn_add or sender == btn_add_ten then
				...
				if selectNums + unitNum > _MAX_USE_NUMS then
					UIManager.showToast(string.format("每次最多使用%d个", _MAX_USE_NUMS))
				else
					if selectNums + unitNum > _haveNums then
						UIManager.showToast("数量不足！")
					else
			...
			elseif sender == btn_sure then
				...
				if selectNums > 0 then
					...
					if fitNums >= selectNums then
						...
						UIManager.showToast(string.format("对应所需要的%s数量不足%d个！", fitName, selectNums))
					end
				else
					UIManager.showToast("请选择数量！")
				end
			end
		end
	end
	...
	ccui.Helper:seekNodeByName(UIBoxUse.Widget, "image_base_earnings_info"):getChildByName("text_hint"):setString(string.format("每次最多使用%d个", _MAX_USE_NUMS))
end
...
function UIBoxUse.setup()
	...
	if _thingData then
		...
		ccui.Helper:seekNodeByName(UIBoxUse.Widget, "text_have_number"):setString("拥有量：".._haveNums)
	end
end
