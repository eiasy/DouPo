...
local function initActivityUI()
    if _activityData then
        for key, obj in pairs(_activityData) do
		    ...
		    item:getChildByName("image_title_di"):getChildByName("text_title_di"):setString(activityTime.startTime[key].."点抢")
		    item:getChildByName("text_price_xian"):setString(string.format("仅剩%d件", _surplusNum))
		    ...
		    if _curTime < _startTime then
			    btn_exchange:setTitleText("未开放")
			    _btnEnabled = false
		    elseif _surplusNum <= 0 then
			    if not isBuy() then
				    btn_exchange:setTitleText("已售罄")
				    _btnEnabled = false
			    end
