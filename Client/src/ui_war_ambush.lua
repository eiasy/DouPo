...
local function setScrollViewItem(item, data)
    ...
    image_job:loadTexture("ui/" ..(data.gradeId == 1 and "lm_mz.png" or(data.gradeId == 2 and "lm_fmz.png" or "lm_cy.png")))
    ...
    local function onButtonEvent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_ambush then
                if btn_ambush:getTitleText() == "设为伏兵" then
                    if ui.ambushCount >= UIWar.ambushMax then
                        UIManager.showToast("伏兵数量已达上限!")
                        return
    ...
    if UIWar.canModify() and UIWar.state == UIWar.STATE_EMBATTLE and data.survive then
        btn_ambush:show()
        btn_ambush:unregisterScriptHandler()
        btn_ambush:addTouchEventListener(onButtonEvent)
        btn_ambush:setTitleText(data.state == UIWar.MEMBER_STATE.AMBUSH and "取消" or "设为伏兵")
    else
        btn_ambush:hide()
    end
    ...
    local function onButtonEvent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == image_di then
                UIAllianceHelp.show( { titleName = "联盟帮助", type = 14 })
...
function ui.setup()
    ...
    image_hint:getChildByName("text_number"):setString(string.format("伏兵人数：%d/%d", ambushCount, UIWar.ambushMax))
    ...
    utils.quickSort(list, function(key, other)
        local a, a1 = getOrder(key)
        local b, b1 = getOrder(other)
