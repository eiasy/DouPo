...
local function freshData()
        ...
        if i <= 9 then
            ...
            if curStars >= DictHoldStarRewardPos[tostring(( 11 - i ))].starNum then
                ...
                if objThingValue[11-i] == 1 then
                    ...
                    image_reward:getChildByName("text_name"):setString("已领取")
    ...
    text_left:setString("剩余刷新次数："..( DictVIP[ tostring( net.InstPlayer.int["19"] + 1 )].holdStarRewardRefreshTimes - freshCount))
    local text_star_number = ccui.Helper:seekNodeByName( UIStarReward.Widget , "text_star_number" )
    text_star_number:setString("当前星数："..curStars)
