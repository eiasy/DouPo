...
--		ui_pillRecipeCount:setString(tostring(pillRecipeCount))
		...
		if dictTableType.id == StaticTableType.DictFightProp then
			...
			ui_pillEffect:setString("丹药功效：永久增加" .. dictFightProp.name .. "+" .. dictPillData.value)
		elseif dictTableType.id == StaticTableType.DictCardBaseProp then
			...
			ui_pillEffect:setString("丹药功效：增加" .. dictCardBaseProp.name .. "+" .. dictPillData.value)
		end
		...
		ui_pillRecipeName:setString("尚未选择丹方")
