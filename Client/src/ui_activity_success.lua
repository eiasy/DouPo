        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            if status == 0 then 
            	if net.InstPlayer.int["4"] >= openLevel then 
	            	...
	            	UIManager.showToast(string.format("%d级开启！",openLevel))
	            end
...
local function setMailScrollViewItem(_Item, _obj)
	...
	elseif _type == 2 then 
		...
		ui_description:setString(string.format("%s击败了您，您的名次滑落到%d名",_obj.string["3"],_obj.int["5"]))
	end
end
