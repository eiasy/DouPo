          ...
          --  cclog( "dictId :".._curCardData.instId )
            ...
            for i = 1 , 8 do
                if i <= getCardTitleId() then
                    local image_name = ccui.Helper:seekNodeByName( sender:getPage(_curPageViewIndex) , "image_name"..i )
                    local text_name = image_name:getChildByName( "text_name" )
                    text_name:setString( "可镶斗魂" )
                    local image_soul = ccui.Helper:seekNodeByName( sender:getPage(_curPageViewIndex) , "image_soul"..i )
                    local panel = image_soul:getChildByName( "panel_soul" )
                    panel:getChildByName( "image_add" ):loadTexture( "ui/frame_tianjia.png" )
                else
            ...
            for key ,value in pairs ( _fightSoul ) do
                local image_name = ccui.Helper:seekNodeByName( sender:getPage(_curPageViewIndex) , "image_name"..value.int[ "8" ] )
                local text_name = image_name:getChildByName( "text_name" )
               ...
               -- utils.changeNameColor( text_name , value.int[ "4" ] , dp.Quality.fightSoul )
                ...
                local image_soul = ccui.Helper:seekNodeByName( sender:getPage(_curPageViewIndex) , "image_soul"..value.int[ "8" ] )
                ...
                utils.addSoulParticle( image_soul:getChildByName("panel_soul") , DictFightSoul[ tostring( value.int[ "3" ] )].effects )
            end
		end
	end
end
...
--				ccui.Helper:seekNodeByName(ui_nameBgImg, "AtlasLabel_36"):setString(tostring(dictCardData.nickname))
            ...
            local name = { "可镶斗魂" , "斗师解锁" , "大斗师解锁" , "斗灵解锁" , "斗王解锁" , "斗皇解锁" , "斗宗解锁" , "斗尊解锁" }
            for i = 1 , 8 do
