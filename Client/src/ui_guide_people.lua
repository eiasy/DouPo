...
local function cleanTaskStep()
    if UIGuidePeople.guideStep == guideInfo["15B6"].step or
        ...
        UIGuidePeople.guideStep == guideInfo["20B14"].step then    
        if UIGuidePeople.guideStep == guideInfo["45B7"].step then
             UIGuidePeople.guideStep = nil
             if UIGuidePeople.levelStep then
                UIGuidePeople.nextCheckLevelGuide = UIGuidePeople.levelStep
               -- print( "UIGuidePeople.nextCheckLevelGuide : %s" , UIGuidePeople.nextCheckLevelGuide )
                UIGuidePeople.levelStep = nil
              --  print( "UIGuidePeople.nextCheckLevelGuide1111 : %s" , UIGuidePeople.nextCheckLevelGuide )
             end
        else
             UIGuidePeople.guideStep = nil
            UIGuidePeople.checkLevelGuide()
...
local function cleanLevelStep()
    if UIGuidePeople.levelStep == guideInfo["7_8"].step or
        ...
        UIGuidePeople.levelStep == guideInfo["11_2"].step or
        ...
        UIGuidePeople.levelStep == guideInfo["28_2"].step or
        UIGuidePeople.levelStep == guideInfo["32_2"].step or
        UIGuidePeople.levelStep == guideInfo["22_3"].step then
        UIGuidePeople.levelStep = nil
    ...
    if UIGuidePeople.levelStep then
        ...
        elseif _level >= guideInfo.duelGuideLevel and utils.beforeLevel < guideInfo.duelGuideLevel then
            if UIGuidePeople.levelStep == guideInfo["11_1"].step then
                if type(data) == "userdata" and uiItem == UIActivityTower then
                    UIGuidePeople.addGuideUI(UIActivityTower, data)
                elseif uiItem == UIFightTask then
                    UIGuidePeople.checkLevelGuide()
                end
            end
        ...
        elseif UIGuidePeople.param == 15 then
            ...
            elseif UIGuidePeople.guideStep == guideInfo["15B5"].step then
                if not data and uiItem == UIFightTaskChoose then
                    local image_base_common = ccui.Helper:seekNodeByName(UIFightTaskChoose.Widget, "image_base_common")
                    UIGuidePeople.addGuideUI(UIFightTaskChoose, ccui.Helper:seekNodeByName(image_base_common, "btn_fight"))
                end
            end
...
local function addPeople(widget_child)
    ...
    if isTest then
        description:setString("我是测试用的，嘟嘟嘟嘟嘟嘟读的嘟嘟嘟 ")
    else
        if UIGuidePeople.guideStep then
