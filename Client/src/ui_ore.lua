...
UIOre = {
    MINE_NAMES = { "翠玉晶矿", "寒冰晶矿", "紫云晶矿", "玄天晶矿" },
    WEATHER_NAMES = { "晴天", "多云", "微风", "小雨", "小雪" },
    MINE_OP_OCCUPY = 0,
    MINE_OP_ASSIST = 1,
    minePageIndex = 1,
    minePageCount = 0,
        ...
        if mine then
            ...
            if mine.minerId ~= 0 then
                ...
                if string.len(mine.alliance) > 0 then
                    text_alliance:show():setString("【联盟" .. mine.alliance .. "】")
...
local function refreshMinePage(weather, pageIndex, mine, mines)
    ...
    local addStr = weatherAddPercent > 0 and string.format("提升%d%%", weatherAddPercent) or(weatherAddPercent == 0 and "无变化" or string.format("降低%d%%", - weatherAddPercent))
    image_common:getChildByName("text_common"):setString(string.format("当前矿收益%s", addStr))
...
local function showImageHint(pageIndex)
    ...
    text_ore_number:setString(string.format("%d号资源区", pageIndex))
...
function ui.showFightResult(uiItem, resultCode, msgdata)
    ...
    if resultCode == 1 then
        info = "矿主威武～恭喜您以绝对的实力夺得了矿点，现在您已经接管了这座资源矿！"
    elseif resultCode == 0 then
        info = "太可惜了，您夺矿失败，闭关努力修炼提升实力吧！"
    else
        info = "被其他高人抢先一步，就慢了那么一点点啦，加油！总会有属于您的矿！"
    end
            ...
            cc.CallFunc:create( function()
                ...
                if _aKeySearchMineResponse then
                    if _aKeySearchMineResponse == 0 then
                        ...
                        UIManager.showToast("已寻找到空的" .. ui.MINE_NAMES[_mineType + 1] .. "，请尽快占领")
                    end
                    _mineType = nil
    ...
    local function touchevent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            if sender == btn_help then
                UIAllianceHelp.show( { titleName = "资源矿帮助", type = 6 })
