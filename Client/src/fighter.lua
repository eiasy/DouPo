...
local NAME_COLOR_TAB = {0x25FF50,0x4ADFFF,0xE17BFF,0xFFBA0D,0xFF0F0F,}
...
function Fighter:setCardID(cardID)
	...
	local imagePath = "image/" .. DictUI[tostring(cardData.bigUiId)].fileName
	if cardData.animationFiles ~= "" then
		local aniFile = cardData.animationFiles
...
function Fighter:setHP(hp,isImmediately)
	...
	self.hpFill:stopAllActions()
	if isImmediately then
		self.hpFill:setScale(self.hp/self.hpMax,1)
	else
		self.hpFill:runAction(cc.ScaleTo:create(0.1,self.hp/self.hpMax,1))
	end
