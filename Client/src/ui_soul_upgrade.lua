...
local function getExp( soulId )
    ...
    if DictFightSoul[ tostring( thing.int["3"] )].isExpFightSoul == 1 then
        ...
        for i = 1 , thing.int[ "5" ] - 1 do
            ...
            exp = exp + DictFightSoulUpgradeExp[ tostring( ( thing.int[ "4" ] - 1 ) * 10 + i ) ].exp
        end 
    end
    return exp  
end
...
local function addExp()
    if _chooseTable and #_chooseTable > 0 and bar_exp_new then
        ...
        bar_exp_new:setPercent( ( exp + _objThing.int[ "9" ] ) * 100 / DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] ) ].exp )
       ...
       -- _objThing.int[ "9" ] * 100 / DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] ) ].exp
        ...
        cclog( "curExp : "..curExp )
        while( curExp > 0 ) do
            curExp = curExp - DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] + lv ) ].exp 
            cclog("-------------->lv :"..( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] + lv .. "    " ..DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] + lv ) ].exp )
            if curExp >= 0 then
                lv = lv + 1
        ...
        bar_exp_new:setPercent( ( _objThing.int[ "9" ] ) * 100 / DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] ) ].exp )
    end
end
...
--            UIManager.pushScene( "ui_soul_info" )
    ...
    utils.addSoulParticle( image_frame_fire:getParent() , DictFightSoul[ tostring( _obj.int[ "3" ] )].effects )
end
...
--                else
                 if #_chooseTable <= 0 then
                     UIManager.showToast( "请点击选择要吞噬的斗魂" )
                 else
                     sendData()
                 end
--                end
...
function UISoulUpgrade.setup()
    ...
    if _objThing then
        ...
        text_quality:setString( "品阶:"..DictFightSoulQuality[ tostring( _objThing.int[ "4" ] ) ].name )
        ...
        if _objThing.int[ "5" ] == 10 then
            text_lv:setString( "等级:LV.MAX" )
        else
            text_lv:setString( "等级:LV.".._objThing.int[ "5" ] )
        end
        ...
        if proValue < 1 then
            text_info:setString( "属性:".."+"..( proValue * 100 ).."%".. DictFightProp[tostring( proType )].name )
        else
            text_info:setString( "属性:".."+"..proValue..DictFightProp[tostring( proType )].name )
        end
       ...
       -- cclog( " " .. _objThing.int[ "5" ] .. "   ".._objThing.int[ "4" ] )
        if _objThing.int[ "5" ] >= 10 then
            ...
            bar_exp:setPercent( _objThing.int[ "9" ] * 100 / DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] ) ].exp )
            bar_exp_new:setPercent( _objThing.int[ "9" ] * 100 / DictFightSoulUpgradeExp[ tostring( ( _objThing.int[ "4" ] - 1 ) * 10 + _objThing.int[ "5" ] ) ].exp )
        end
        ...
        utils.addSoulParticle( image_black:getChildByName("panel") , DictFightSoul[ tostring( _objThing.int[ "3" ] )].effects )
    end
    _soulData = {}
        ...
        while #_chooseTable < 50 and index <= #_choose do
            for key , value in pairs ( _soulData ) do
                ...
                if #_chooseTable >= 50 then
                    UIManager.showToast("一次最多选择50个斗魂")
                    break
                end
            end
            index = index + 1
