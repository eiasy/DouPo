...
local function setScrollViewItem(_Item, _obj)
    ...
    if inTable(0, tonumber(_obj.int.id)) == 1 then
        ...
        if starTime and endTime then
            ui_time:setString(string.format("活动时间：%d/%d %d点--%d/%d %d点", starTime[2], starTime[3], starTime[5], endTime[2], endTime[3], endTime[5]))
        end
    ...
    if chargeGold >= _obj.int.progress then
        ui_num:setString(string.format("进度：%d/%d", _obj.int.progress, _obj.int.progress))
        if isGet then
        ...
        ui_num:setString(string.format("进度：%d/%d", chargeGold, _obj.int.progress))
        btn_exchange:setTitleText("去充值")
        btn_exchange:setEnabled(true)
        utils.GrayWidget(btn_exchange, false)
    end
