...
--	local name = dictObj.name
	...
	ui_name:setString("积分×" .. dictObj.plan)
       		...
       		if  rewardState == 0 and rewardTimes < totalTimes then 
                local taskLevel = DictFunctionOpen[tostring(dictObj.functionOpenId)].level
       		    if net.InstPlayer.int["4"] < taskLevel then
                    return UIManager.showToast("需要战队等级达到" .. taskLevel .. "级！")
                end 
       			sender:retain()
	       		...
	       		elseif dictObj.sname == "skillChipLoot" then --抢碎片
				    ...
				    if lootOpen then 
						...
						UIManager.showToast("需要通关关卡魔兽山脉边缘的“火狼袭击”")
						return  
					end
...
--			    end
        ...
        if boxData[key] then
            ...
            obj.box:addTouchEventListener(function(sender, eventType)
                if eventType == ccui.TouchEventType.ended then
                    ...
                    UIAwardGet.setOperateType(UIAwardGet.operateType.dailyTaskBox, {btnTitleText=_btnTitleText, enabled=_enabled, 
                        things=dictObj.rewards, callbackFunc=function() _awardData=utils.stringSplit(dictObj.rewards, ";"), sendRewardData(boxData[key].int["1"]) end})
	   			    UIManager.pushScene("ui_award_get")
                end
            end)
        end
    ...
    ui_curValueLabel:setString("当前积分：" .. currentValue)
