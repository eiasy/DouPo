...
local function setPagodaInfo()
	...
	_storeyName:setString("第".._curFightStoreyId.."层")
    if data then
	    local _text = ""
	    if data.victoryMeans == 1 then
		    _text = "战斗回合数不超过" .. data.victoryValue
	    elseif data.victoryMeans == 2 then
		    _text = "死亡卡牌数不超过" .. data.victoryValue
	    elseif data.victoryMeans == 3 then
		    _text = "战斗结束后血量不少于" .. data.victoryValue .. "%"
	    elseif data.victoryMeans == 4 then
		    _text = "消灭全部敌人"
	    end
	    _fightNums:setString(string.format("挑战次数%d/3", net.InstPlayerPagoda.int["4"]))
	    _passCondition:setString("通关条件：" .. _text)
...
local function onBtnResetEvent()
	...
	if net.InstPlayerPagoda.int["5"] < resetNum then
		...
		showDialog("重置将恢复您的挑战次数，并重置回第1层，确定重置？")
	else
		...
		if eventType == ccui.TouchEventType.ended then
			...
			elseif sender == btn_sweep then
				if btn_sweep:isBright() then
					...
					UIManager.showToast("至少要通关1层才能开启扫荡")
				end
			elseif sender == btn_zhan then
                if DictPagodaStorey[tostring(_curFightStoreyId)] then
				    ...
				    if net.InstPlayerPagoda.int["4"] == 0 then
					    showDialog("挑战次数不足，是否进行重置？", onBtnResetEvent)
					    return
				    end
                    ...
                    UIManager.showToast("此层尚未开放，敬请期待！")
                end
...
function UITowerTest.setup()
	...
	if resetNum - net.InstPlayerPagoda.int["5"] == 0 then
		ui_resetNum:setString("次数已用完")
	else
		ui_resetNum:setString("免费重置次数：" .. (FREE_RESET_COUNT - net.InstPlayerPagoda.int["5"]))
	end
	if net.InstPlayerPagoda.int["5"] >= FREE_RESET_COUNT and resetNum - net.InstPlayerPagoda.int["5"] > 0 then
	...
	-- ui_searchNum:setString("今日次数：" .. (searchNum - net.InstPlayerPagoda.int["6"]) .. "/" .. searchNum)
	ui_sweepNum:setString("最高层数：" .. net.InstPlayerPagoda.int["7"])
	...
	if _isPass ~= nil then
		...
		if _isPass then
			...
			_curFightStoreyId = net.InstPlayerPagoda.int["3"]
			ui_image_base_task:getChildByName("text_challenge_number"):setString(string.format("挑战次数%d/3", net.InstPlayerPagoda.int["4"]))
