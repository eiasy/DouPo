...
function UILootHint.init()
   ...
   btn_gold_buy:setTitleText(string.format("消耗：%d元宝",DictSysConfig[tostring(StaticSysConfig.lootGold)].value))
   ui_text_peace_cost:setString(string.format("消耗一个和平牌，或者%d元宝",DictSysConfig[tostring(StaticSysConfig.lootGold)].value))
end
function UILootHint.setup()
   if operateType == UILootHint.operateType.chip then 
      ...
      if type == 2 then 
        description = qualityType .. "功法"
      elseif type == 3 then 
        description = qualityType .. "法宝"
      end
