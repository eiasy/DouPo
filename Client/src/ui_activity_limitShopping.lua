...
local function countDowun()
    ...
    if UIActivityLimitShopping.Widget then
        ...
        image_basemap:getChildByName("text_time_left"):setString(string.format("剩余时间 %d天%02d:%02d:%02d", day, hour, minute, second))
    end
end
...
local function initActivityUI()
    if _activityData then
        for key, obj in pairs(_activityData) do
		    ...
		    item:getChildByName("text_price_xian"):setString("限购次数：" .. obj.limitNum)
...
--					    end
        ...
        if DictActivity then
		    ...
		    local image_basemap = UIActivityLimitShopping.Widget:getChildByName("image_basemap")
            image_basemap:getChildByName("text_time"):setString(string.format("活动时间：%d月%d日-%d月%d日", _startTime[2], _startTime[3], _endTime[2], _endTime[3]))
