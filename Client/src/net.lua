...
local function onCheckVersion()
    if http and http.status == 200 then
        ...
        if downloadResourceVersion > localResourcesVersion then
            showPromptDialog("检测到有资源更新，请重新启动游戏更新资源")
...
function netOnConnectFailed()
    ...
    cclog("-------->>>  socket连接失败！")
    if _disconnect then
        utils.netErrorDialog("您的网络异常，请尝试重新连接")
    elseif _isCloseServer then
        UIManager.showToast("服务器停服维护中，请稍后再试")
    else
        UIManager.showToast("网络连接失败，请检查您的网络后再试")
    end
end
...
function netOnDisconnected()
    ...
    if not _isCloseServer then
        if _isErrorLogin then
            utils.netErrorDialog("您的账号在异地登陆", true)
            _isErrorLogin = false
        else
            _disconnect = true
            utils.netErrorDialog("您的网络异常，请尝试重新连接")
...
function netSendPackage(pack, netCallbackFunc, errorCallbackFunc, isShowErrorToastMsg)
...
function netOnPackage(pack)
    ...
    if pack.msgdata.int and tonumber(pack.msgdata.int.isOk) == 0 then
        if pack.msgdata.string.retMsg ~= "" then
            if pack.msgdata.string.retMsg == "账号在异地登录" then
                _isErrorLogin = true
            elseif pack.msgdata.string.retMsg == ":" then
                -- 重连时验证Token失效了
                utils.netErrorDialog("连接失败，请重新登录", -1)
            else
                if pack.msgdata.string.retMsg == "服务器爆满!" then
                    netDisconnect()
                else
