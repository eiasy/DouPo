...
if SDK.getChannel() == "anysdk" then
	...
	cc.Director:getInstance():getScheduler():scheduleScriptFunc(
		function(dt)
			...
			if lastClock > 2140 and currClock < -2140 then --todo未完整测试
				lastClock = currClock
				return
			end
			...
			if factor > 1.1 then
				...
				if happenTimes > 5 then
					...
					msgLabel:setString("为了保证游戏体验，请勿使用外挂！\n点击确认退出游戏！")
	...
	,1,false)
	end --todo 解决IOS，暂不缩进了，Android发现也有问题。
...
local function updateAllSchduleAction()
	if utils.intervalTime > 0 then
		...
		if UIFightTaskChoose.Widget ~= nil then
			UIFightTaskChoose.setTimeInterval(utils.intervalTime)
		end
