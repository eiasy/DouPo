...
local function netCallbackFunc(pack)
  if tonumber(pack.header) == StaticMsgRule.sellCards then
    UIManager.popScene()
    UIManager.showToast("恭喜您获得" .. _totalNumber .. "银币")
...
local function setScrollViewItem(item, data)
	...
	ui_cardLevel:setString(string.format("等级：%d",cardLv))
		...
		if eventType == ccui.CheckBoxEventType.selected then
			if _operateType == UIBagCardSell.OperateType.CardSell then
				if #_selectedInstCardIds == 30 then 
					UIManager.showToast("一次最多只能出售30")
					ui_checkBox:setSelected(false)
					return 
				end
				cclog("---------->>>  选择")
          		...
          		ui_chooseNum:setString("已选强者：" .. #_selectedInstCardIds)
          		ui_sellNum:setString("总计出售：" .. _totalNumber)
			elseif _operateType == UIBagCardSell.OperateType.CardUpgrade then
                
				if #_selectedInstCardIds >= 5 then
					UIManager.showToast("选择数量已上限")
					ui_checkBox:setSelected(false)
				else
					if not isContain(instCardId) then
						_selectedInstCardIds[#_selectedInstCardIds + 1] = instCardId
					...
					ui_chooseNum:setString("已选强者：" .. #_selectedInstCardIds)
					ui_sellNum:setString("获得经验：" .. _totalNumber)
				end
			end
...
function UIBagCardSell.setup()
	...
	cardThing={}
	if _operateType == UIBagCardSell.OperateType.CardSell then
		ui_titleText:setString("卡牌出售")
		ui_chooseNum:setString("已选强者：0")
		ui_sellNum:setString("总计出售：0")
		ui_btnSell:setTitleText("出售")
	elseif _operateType == UIBagCardSell.OperateType.CardUpgrade then
		ui_titleText:setString("卡牌列表")
		ui_chooseNum:setString("已选强者：0")
		ui_sellNum:setString("获得经验：0")
		ui_btnSell:setTitleText("确定")
		if _param then
