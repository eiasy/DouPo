...
local function getThingFunc(data)
    ...
    if data ~= nil then
        ...
        for key, value in pairs(_getSoulThing) do
            ...
            local _description = ""
            if proValue < 1 then
                _description = "属性:" .. "+" ..(proValue * 100) .. "%" .. DictFightProp[tostring(proType)].name
            else
                _description = "属性:" .. "+" .. proValue .. DictFightProp[tostring(proType)].name
            end
            ...
            local function show(Enable)
                if Enable then
                    ...
                    utils.addSoulParticle(image, imageName)
                    local description = ccui.Text:create()
                    local height = math.floor(utils.utf8len(_description) -1) / 18 + 1
                    description:setTextAreaSize(cc.size(360, height * 25))
                    description:setFontSize(20)
            ...
            utils.addSoulParticle(getThingImage[index]:getParent(), imageName)
...
local function netRinneiCallback(pack)
    ...
    elseif pack.header == StaticMsgRule.restoreCard then
...
local function addFunction(flag)
    ...
    if flag == 1 then
        ...
        if resolveList == "" then
            if pEquip then
                UIManager.showToast("橙色装备需手动添加！")
            else
                UIManager.showToast("没有装备可添加！")
            end
    ...
    elseif flag == 2 then
        ...
        if resolveList == "" then
            if pCard then
                UIManager.showToast("橙色卡牌需手动添加！")
            else
                UIManager.showToast("没有卡牌可添加！")
            end
            UIResolve.Resolveclear()
        else
            sendReViewdData(addFlag, resolveList)
    ...
    elseif flag == 4 then
        ...
        if resolveList == "" then
            if pMagic then
                UIManager.showToast("橙色法宝需手动添加！")
            else
                UIManager.showToast("没有法宝可添加！")
            end
            UIResolve.Resolveclear()
        else
            sendReViewdData(addFlag, resolveList)
...
local function btn_resolveFunc()
    ...
    if addFlag == 1 and next(addEquip.info) then
        if addEquip.flag then
            local textStr = "你要分解的装备中，有绿色以上等级的，确定分解？"
    ...
    elseif addFlag == 2 and next(addCard.info) then
        if not UIGuidePeople.levelStep then
            if addCard.flag then
                local textStr = "你要分解的卡牌中，有蓝色以上等级的，确定分解？"
                utils.PromptDialog(sendResolvedData, textStr, instResolveId)
            else
                sendResolvedData(instResolveId)
            end
...
function UIResolve.init()
    ...
    local text_hint = ccui.Helper:seekNodeByName(UIResolve.Widget, "text_hint")
...
function UIResolve.setup()
    ...
    if _operateType == nil then
        ...
        ui_image_card_big:setVisible(false)
        ui_image_base_name:setVisible(false)
        ui_image_base_tab:getChildByName("btn_resolve"):setTitleText("分解")
        getThingFunc()
        for i = 1, 5, 1 do
            img_resolve_add[i]:setVisible(true)
            local temp = ccui.Helper:seekNodeByName(UIResolve.Widget, "image_resolve_good" .. i)
...
function UIResolve.setRinneData(_InstPlayerCardObj)
