...
local function propThing( obj )
    ...
    image_name:getChildByName("text_name"):setString("羽毛名字")
    ...
    ccui.Helper:seekNodeByName( image_down , "text_name" ):setString("羽毛品阶")
...
--        text_name:setString( "属性："..i )
    ...
    if thingData then
        ...
        if DictTitleDetail[tostring(titleDetailId)].titleId >= levelId then
            ccui.Helper:seekNodeByName( image_down , "text_name" ):setString("未佩戴神羽")
            btn_up:setVisible( false )
        else
            btn_up:setVisible( true )
            local dictTitleDetailData = DictTitle[tostring(levelId)]
            ccui.Helper:seekNodeByName( image_down , "text_name" ):setString( dictTitleDetailData.name.."境界才可佩戴神羽")
        end
        for i = 1 , 8 do        
            local text_name = image_wing_di:getChildByName( "text_name"..i )
