...
local function setListData(data)
    ...
    image_basemap:getChildByName("text_rank"):setString("当前排名：" .. data.msgdata.int.r2)
...
initScrollViewItem = function(_item, _data)
    ...
    if _data then
        ...
        item_playerLevel:setString("等级：" .. _data.level)
        if _data.allianceName and _data.allianceName ~= "" then
            item_allianceName:setString("联盟：" .. _data.allianceName)
        else
            item_allianceName:setString("")
        end
        item_maxPoint:setString(string.format("最高层：%d层", _data.maxPoint))
...
function UIPilltowerRank.setup()
    ...
    image_basemap:getChildByName("text_lv"):setString("等级：" .. net.InstPlayer.int["4"])
    image_basemap:getChildByName("text_rank"):setString("当前排名：" .. 0)
end
