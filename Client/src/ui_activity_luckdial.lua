...
local function refreshTime(start, stop)
    ...
    local time = "活动时间：" .. startTime.month .. "月" .. startTime.day .. "日" .. startTime.hour .. "点"

    local time = string.format("活动时间：%d月%d日%d点", startTime.month, startTime.day, startTime.hour)
    if (startTime.min > 0) then
        time = time .. startTime.min .. "分"
    end
    time = time .. string.format("-%d月%d日%d点", endTime.month, endTime.day, endTime.hour)
    if (endTime.min > 0) then
        time = time .. endTime.min .. "分"
    end
...
local function refreshLimit(limit)
    ...
    for i, thing in ipairs(limit) do
        local itemProps = utils.getItemProp(thing)
        setIcon(image_rank_title:getChildByName("image_frame_good" .. i), itemProps, "剩余:", true)
    end
end
...
local function showTenReward(awards)
    ...
    ccui.Helper:seekNodeByName(UIBoxGet.Widget, "text_get"):setString("恭喜您获得如下物品")
...
local function netCallbackFunc(pack)
    ...
    if code == StaticMsgRule.activityLuck then
        ...
        if startRemain > 0 then
            btn_one:getChildByName("text_hint"):show():setString(string.format("今日免费次数：%d", startRemain))
        ...
        text_price:setString(refreshRemain > 0 and string.format("免费%d次", refreshRemain) or string.format("%d元宝", refreshCost))
    ...
    local function touchevent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_help then
                UIAllianceHelp.show( { titleName = "活动详情", type = 3 })
