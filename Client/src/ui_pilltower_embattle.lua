    ...
    for i = 1, 9 do
        ...
        if item:getTag() > 0 then
            ...
            if touchPoint.x > itemPoint.x - item:getContentSize().width / 2 and touchPoint.x < itemPoint.x + item:getContentSize().width / 2 and
                ...
                _curTouchCard:getChildByName("image_card"):loadTexture("image/" .. DictUI[tostring(dictCard.bigUiId)].fileName)
                local totalBlood = utils.getCardAttribute(lineupData.instCardId)[StaticFightProp.blood]
...
local function initMonsterUI()
    ...
    for i = 1, 9 do
        ...
        if enemyLineupData[i] then
            ...
            _item:loadTexture(utils.getQualityImage(dp.Quality.card, dictCard.qualityId, dp.QualityImageType.small))
...
function UIPilltowerEmbattle.init()
    ...
    image_basemap:getChildByName("text_title"):setString(string.format("第%d层战前布阵", UIPilltower.UserData.curFightPoint))
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_earlier then
                if UIPilltower.UserData.medalCount >= firstUseCount then
                    ...
                    UIManager.showToast("您的勋章数不足！")
                end
            elseif sender == btn_blood then
                for i = 1, 9 do
                    local _data = UIPilltower.UserData.myCardData[i]
                    if _data then
                        local totalBlood = utils.getCardAttribute(_data.instCardId)[StaticFightProp.blood]
                ...
                UIManager.showToast("您动力十足不需要补给")
                            ...
                            for i = 1, 9 do
                                ...
                                if myCardData then
                                    ...
                                    if _blood then
                                        ...
                                        local totalBlood = utils.getCardAttribute(myCardData.instCardId)[StaticFightProp.blood]
...
--                            UIManager.showToast("战斗失败！~")
                ...
                elseif #myData < DictDantaLayer[tostring(userData.monsterId)].maxSubstituteCount + DictDantaLayer[tostring(userData.monsterId)].maxFirstCount then
                    utils.showDialog("您的上阵英雄人数没有达到上限，确定开始挑战？", netSendFightPackage)
                else
                    netSendFightPackage()
                end
            end
    ...
    myLineupPanel:getChildByName("text_hint"):setString(string.format("首发最多%d人，替补最多%d人", dictData.maxFirstCount, dictData.maxSubstituteCount))
end
...
function UIPilltowerEmbattle.setup()
    ...
    for key = 1, 9 do
        ...
        if lineupData then
            ...
            ui_cardIcon:loadTexture("image/" .. DictUI[tostring(dictCard.bigUiId)].fileName)
            local totalBlood = utils.getCardAttribute(lineupData.instCardId)[StaticFightProp.blood]
            local currentBlood = (lineupData.cardBlood > totalBlood) and totalBlood or lineupData.cardBlood
...
function UIPilltowerEmbattle.refreshCardBlood()
    ...
    for key = 1, 9 do
        ...
        if cardIconData then
            local totalBlood = utils.getCardAttribute(cardIconData.instCardId)[StaticFightProp.blood]
