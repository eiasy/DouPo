...
local function expalain()
    ...
    description:setString("1、转换100%成功 \n2、放入需要转换的魔核后，将显示可转换的魔核类型\n3、需要手动转换魔核的类型\n")
    ...
    local function btnEvent(sender,eventType)
      if eventType == ccui.TouchEventType.ended then
          ...
          elseif sender == btn_switch then 
              if not obj then 
                UIManager.showToast("请添加魔核！") 
                return
              end
              if chooseImage == nil then 
                    UIManager.showToast("未选择转换的类型！") 
                    return;
              end
              if number == 0 then 
                    UIManager.showToast("请选择要转换魔核的数量！") 
                    return;
              end
               if cost_money <= net.InstPlayer.int["5"] then 
                  ...
                  UIManager.showToast("您的银币不足，无法转换魔核！")
               end
        ...
        local function ImageEvent(sender,eventType)
             ...
             if eventType == ccui.TouchEventType.ended and LongPush == false then
                 ...
                 if changedPropId ~= nil then 
                     ...
                     prompt:setString("您选择的魔核类型：" .. _name)
                 else
                    prompt:setString("请选择要转换的魔核类型")
                 end
