...
local function setScrollViewItem(_item, _data, key)
    ...
    text_count:setString("剩余次数: " .. _data.count)
    local _tableStartTime = os.date("*t", _data.startTime / 1000)
    local _tableEndTime = os.date("*t", _data.endTime / 1000)
    text_time:setString(string.format("兑换时间 ：%d月%d日%d点 - %d月%d日%d点", _tableStartTime.month, _tableStartTime.day, _tableStartTime.hour, _tableEndTime.month, _tableEndTime.day, _tableEndTime.hour))
