...
local function setScrollViewItem(_Item, _obj)
    ...
    if _obj.int[ "4" ] == 5 then
        text_describe:setString( "出售可获得"..sellSilver.."银币" )
    elseif _obj.int[ "4" ] == 4 and DictFightSoul[ tostring( _obj.int[ "3" ] ) ].isExpFightSoul == 1 then
        text_describe:setString( "吞噬可获得"..DictFightSoul[ tostring( _obj.int[ "3" ] ) ].initExp.."经验")
    else
        if proValue < 1 then
    ...
    if _obj.int[ "7" ] == 0 then
        ...
        text_for:setString("装备于"..cardName )
    end
    ...
    utils.addSoulParticle( image_frame_soul:getChildByName( "image_soul" ) , DictFightSoul[ tostring( _obj.int[ "3" ] )].effects )
end
