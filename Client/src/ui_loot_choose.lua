...
local guide =nil 
---1 pc 2 npc
local function setScrollViewItem(type,playerItem, _obj)
    ...
    ui_label_lv:setString(_obj.int["2"].. "级")
    local imageName = nil
    if type == 1 then
      ...
      if UILootChoose.ds[tostring(type)] == 3 then 
        imageName = "gl_ybgl.png"
      elseif UILootChoose.ds[tostring(type)] == 4 then 
        imageName = "gl_dgl.png"
      elseif UILootChoose.ds[tostring(type)] == 5 then 
        ...
        if eventType == ccui.TouchEventType.ended then
            ...
            if type == 1 then 
              ...
              if tonumber(curHour) >= startTime and tonumber(curHour) < (tonumber( endTime ) + 1) then 
                UIManager.showToast(string.format("每日%02d:00至%02d:00无法抢夺其他玩家",startTime,tonumber( endTime ) + 1 ) )
            ...
            if net.InstPlayer.int["10"] >= DictSysConfig[tostring(StaticSysConfig.lootVigor)].value then
                UILootChoose.warParam ={} 
                table.insert(UILootChoose.warParam,type)
                ...
                if type == 1 or UIGuidePeople.guideStep == guideInfo["20B6"].step then
                    if UILoot.ScheduleId  ~= nil then
                        local info = "在和平镇保护期间，如若抢夺其他玩家将会解除保护时间，是否继续抢夺？"
    ...
    if type == 2 then
        if UIGuidePeople.guideStep == guideInfo["20B5"].step then
            btn_loot:setTitleText("抢夺")
        else
            btn_loot:setTitleText("一键抢夺")
        end
    else
        btn_loot:setTitleText("抢夺")
    end
...
function UILootChoose.init()
    ...
    ui_cost:setString("夺宝消耗：" .. DictSysConfig[tostring(StaticSysConfig.lootVigor)].value)
    ui_hint:setString(string.format("每日%02d:00至%02d:00无法抢夺其他玩家",DictSysConfig[tostring(StaticSysConfig.lootCloseStartTime)].value, ( tonumber ( DictSysConfig[tostring(StaticSysConfig.lootCloseEndTime)].value) ) + 1 ) )
end
...
function UILootChoose.setup()
    ...
    ui_endurance2:setString("耐力：" .. net.InstPlayer.int["10"] .. "/" .. net.InstPlayer.int["11"])
