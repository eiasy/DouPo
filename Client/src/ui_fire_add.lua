...
local function setFireUI(_cardInstId)
    ...
    ui_fireTitleName:setString("点击下方异火栏位装备" .. userData.fireData.DictYFire.name)
    ...
    for key, objUI in pairs(fireUI) do
        ...
        if _gridState == 0 then
            ...
            if DictQuality[tostring(dp.FireEquipGrid[key].qualityId)] == nil then
                _openText = "金品" .. DictStarLevel[tostring(dp.FireEquipGrid[key].starLevelId)].name
            else
            ...
            --/////////// 暂时写死，最大开放到红品5阶 ////////////
            fireUI[key].ui_fireText:setString(_openText .. "\n可开启")
        ...
        if InstPlayerYFire then
            ...
            if _gridState ~= 1 then
                ...
                fireUI[key].ui_fireText:setString("实力不足\n无法使用此异火")
                fireUI[key].ui_fireIcon:setTouchEnabled(false)
            else
            ...
            if eventType == ccui.TouchEventType.ended then
                ...
                for _i, _obj in pairs(_equipFireInstData) do
                    if _obj.int["3"] == userData.fireData.DictYFire.id then
                        UIManager.showToast("您已经装备了此种异火，不能重复装备！")
                        return
                    end
                end
    ...
    for key, obj in pairs(CustomDictYFireProp) do
        ...
        if DictQuality[tostring(obj.qualityId)] == nil then
            _openText = "金品" .. DictStarLevel[tostring(obj.starLevelId)].name
        else
        ...
        ui_fireNum:setString(string.format("装备%d种异火", obj.equipFireCount))
        ...
        for _k, _o in pairs(fightPropId) do
            if tonumber(_o) == StaticFightProp.blood then
                _texts[#_texts + 1] = DictFightProp[_o].name .. "上限提高" .. fightPropValue[_k] .. "%"
            else
                if tonumber(_o) == StaticFightProp.wDefense or tonumber(_o) == StaticFightProp.fDefense then
                    _texts[#_texts + 1] = DictFightProp[_o].name .. "提高" .. fightPropValue[_k] .. "%"
                end
            end
        end
        local _stringText = ""
...
function UIFireAdd.init()
    ...
    text_title:setString("装备" .. (userData and userData.fireData.DictYFire.name or "异火"))
end
