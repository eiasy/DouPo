...
function UIGemInlay.setup()
	...
	if net.InstPlayerEquip and _equipInstId then
		...
		ui_equipLevel:setString("等级：" .. equipLv)
			...
			if key <= holeNum then
				...
				if _thingId then
					if _thingId == 0 then
						...
						btnItem:setTitleText("镶 嵌")
						_gemHint:setString("可放入魔核，提升战斗力")
					...
					_gemLv:setVisible(false)
					_gemProp:setVisible(false)
					btnItem:setTitleText("打 孔")
					_gemHint:setString("消耗" .. holeNums[key] .. "个打孔器")
