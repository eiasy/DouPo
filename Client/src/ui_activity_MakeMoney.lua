...
local function showGetGold(gold)
	...
	text_closed:setString("点击屏幕任意区域关闭！")
...
function ui.setup()
	...
	if DictActivity and DictActivity.string["4"] ~= "" and DictActivity.string["5"] ~= "" then
		...
		text_time_open:setString(string.format("活动时间：%d月%d日%d点", _startTime[2],_startTime[3],_startTime[5]))
		text_time_off:setString(string.format("——%d月%d日%d点",_endTime[2],_endTime[3],_endTime[5]))
	end
end
