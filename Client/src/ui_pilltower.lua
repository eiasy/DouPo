...
--            UIPilltower.UserData.myCardData[i].cardId = obj.int["6"]
            ...
            UIPilltower.UserData.myCardData[i].cardBlood = utils.getCardAttribute(obj.int["3"])[StaticFightProp.blood]
...
--                    local _monsterId = utils.stringSplit(DictDantaLayer[tostring(_dantaLayerId)].monsters, ",")[1]
                            ...
                            pointItem:addTouchEventListener(function(sender, eventType)
                                if eventType == ccui.TouchEventType.ended then
                                    if type(UIPilltower.UserData.challengeNums) == "number" and UIPilltower.UserData.challengeNums > 0 or _curFightPoint ~= 1 then
                                        ...
                                        UIManager.showToast("今日挑战次数已用完，改日再战吧！")
                                    end
                                end
                            end)
                        end
                ...
                ccui.Helper:seekNodeByName(pointItem, "text_name"):setString("第" .. _curFightStartPointId .. "层")
    ...
    local function onClickEvent(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            elseif sender == btn_help then
                UIAllianceHelp.show({type=5,titleName="丹塔帮助"})
            ...
            elseif sender == btn_rank then
                if UIPilltower.UserData.isDebug then
                    UIManager.showToast("暂无数据！")
                else
                    UIPilltowerRank.show()
                end
            elseif sender == btn_reset then
                if UIPilltower.UserData.curFightPoint == 1 then
                    UIManager.showToast("无需重置！")
                    return
                end
                utils.showDialog("您的挑战还没有失败，是否确定重置丹塔？", function()
            ...
            elseif sender == btn_again then
                if UIPilltower.UserData.historyMaxPoint and UIPilltower.UserData.historyMaxPoint >= Encrypt.decryptNumber(MOP_UP_OPEN_CONDITION) then
                    if UIPilltower.UserData.curFightPoint ~= 1 then
                        UIManager.showToast("扫荡功能只能在挑战第一层之前使用！")
                        return
                    end
                    ...
                    UIManager.showToast(string.format("历史战绩到达%d层开启扫荡功能！", Encrypt.decryptNumber(MOP_UP_OPEN_CONDITION)))
                end
            end
        end
    end
...
function UIPilltower.setup(_callback)
    ...
    ui_floorNum:setString(string.format("历史最高%d层", UIPilltower.UserData.historyMaxPoint and UIPilltower.UserData.historyMaxPoint or 0))
    ...
    if _challengeNums == 0 then
        ui_resetNum:setString("今日挑战次数已用完")
    else
        ui_resetNum:setString(string.format("还有%d/%d次挑战机会", _challengeNums, DictSysConfig[tostring(StaticSysConfig.DanTaNum)].value))
    end

    local ui_againText = image_di_reset:getChildByName("text_again")
    if UIPilltower.UserData.historyMaxPoint and UIPilltower.UserData.historyMaxPoint >= Encrypt.decryptNumber(MOP_UP_OPEN_CONDITION) then
        if UIPilltower.UserData.curFightPoint == 1 then
            ui_againText:setString("只能在挑战第一层之前使用")
        else
            ui_againText:setString(string.format("下一阶段扫荡%d层开启", nextMopUpCondition()))
        end
    else
        ui_againText:setString(string.format("历史战绩到达%d层开启", Encrypt.decryptNumber(MOP_UP_OPEN_CONDITION)))
    end
    ...
    if (not UIPilltower.UserData.historyMaxPoint) or (not UIPilltower.UserData.challengeNums) then
        UIPilltower.netSendPackage({int={p2=1}}, function(_msgData)
            ...
            ui_floorNum:setString(string.format("历史最高%d层", UIPilltower.UserData.historyMaxPoint))
            if UIPilltower.UserData.challengeNums == 0 then
                ui_resetNum:setString("今日挑战次数已用完")
            else
                ui_resetNum:setString(string.format("还有%d/%d次挑战机会", UIPilltower.UserData.challengeNums, DictSysConfig[tostring(StaticSysConfig.DanTaNum)].value))
            end
...
function UIPilltower.exitWaring(_callFunc)
    if UIPilltower.Widget and UIPilltower.Widget:getParent() then
        if UIPilltower.UserData.curFightPoint > 1 then
            utils.showDialog("一旦退出，将失去此次挑战机会，确定退出？", function()
                UIPilltower.netSendPackage({int={p2=4}}, function(_msgData)
