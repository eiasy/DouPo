...
function UIAlliance.init()
	...
	local btn_hall = ccui.Helper:seekNodeByName(image_di_system, "btn_hall") --议事大厅
	local btn_shop = ccui.Helper:seekNodeByName(image_di_system, "btn_shop") --联盟商店
	local btn_build = ccui.Helper:seekNodeByName(image_di_system, "btn_build") --联盟建设
	local btn_wait = ccui.Helper:seekNodeByName(image_di_system, "btn_wait") --即将开放
...
local function getTime(_secsNums)
	local _year = math.floor(_secsNums / 3600 / 24 / 30 / 12 % 12) --年
	local _month = math.floor(_secsNums / 3600 / 24 / 30 % 30) --月
	local _week = math.floor(_secsNums / 3600 / 24 / 7 % 7) --周
	local _day = math.floor(_secsNums / 3600 / 24 % 24) --天
	local _hour = math.floor(_secsNums / 3600 % 24) --小时
	local _minute = math.floor(_secsNums / 60 % 60) --分
	local _second = math.floor(_secsNums % 60) --秒
	...
	if net.InstUnionMember then
		for key, obj in pairs(DictUnionGrade) do
			if _allianceGradeType == obj.type and _gradeId == obj.gradeId then
				local flagImg = ""
				if _gradeId == 1 then flagImg = "ui/lm_mz.png"
				elseif _gradeId == 2 then flagImg = "ui/lm_fmz.png"
				elseif _gradeId == 3 then flagImg = "ui/lm_cy.png" end
