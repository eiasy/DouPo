...
local function setScrollViewItem(_item, _data)
    ...
    ui_buyCount:setString("购买个数：" .. _data.buyCount)
    if _data.backCount > 0 then
        ui_backText:setString(string.format("返利\n%d", _data.backCount * 100) .. "%")
...
function UIActivityPurchaseRank.setup()
    ...
    image_di:getChildByName("text_number"):setString("购买个数：" .. userData.buyCount)
    image_di:getChildByName("text_rank"):setString("我的排名：" .. userData.currentRank)
