    ...
    void ButtonReader::setPropsWithFlatBuffers(cocos2d::Node *node, const flatbuffers::Table *buttonOptions)
    {
        ...
        switch (normalType)
        {
            ...
            case 1:
            {
				...
				std::string::size_type pos = plist.find(".plist");
				plist.replace(pos, plist.length(), ".md");
		...
		std::string customProperty = options->widgetOptions()->customProperty()->c_str();

		if (!customProperty.empty()) {
			auto lr = button->getTitleRenderer();

			if (customProperty.find("bold") != std::string::npos) {
				auto ttfConfig = lr->getTTFConfig();
				ttfConfig.isBold = true;
				lr->setTTFConfig(ttfConfig);
			}

			size_t index = customProperty.find("outlineColor=#");
			if (index != std::string::npos) {
				auto str = customProperty.substr(index + strlen("outlineColor=#"), 6);
				char *endPtr;
				auto color = strtol(str.c_str(), &endPtr, 16);
				int r = (color >> 16) & 0xFF;
				int g = (color >> 8) & 0xFF;
				int b = color & 0xFF;

				index = customProperty.find("outlineSize=");
				str = customProperty.substr(index + strlen("outlineSize="), 1);

				int outlineSize = atoi(str.c_str());

				lr->enableOutline(Color4B(r, g, b, 255), outlineSize);
			}

			index = customProperty.find("gradientColor=#");
			if (index != std::string::npos) {
				auto str = customProperty.substr(index + strlen("gradientColor=#"), 6);
				char *endPtr;
				auto color = strtol(str.c_str(), &endPtr, 16);
				int r = (color >> 16) & 0xFF;
				int g = (color >> 8) & 0xFF;
				int b = color & 0xFF;
				lr->setGradientColor(Color4B(r, g, b, 255));
			}
		}
        ...
        else
        {
			...
			button->ignoreContentAdaptWithSize(options->widgetOptions()->ignoreSize());
        }
    }
