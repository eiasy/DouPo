    ...
    Offset<Table> WidgetReader::createOptionsWithFlatBuffers(const tinyxml2::XMLElement *objectData, flatbuffers::FlatBufferBuilder *builder)
    {
        ...
        while (attribute)
        {
            ...
            else if (attriname == "UserData")
            {
				...
				int pos = value.find("ignoreSize");
				if (pos != std::string::npos)
				{
					ignoreSize = true;
					int count = strlen("ignoreSize");
					bool removed = false;
					if (pos > 0 && customProperty[pos - 1] == ',')
					{
						--pos;
						++count;
						removed = true;
					}
					if (!removed && pos < customProperty.size() - 1 && customProperty[pos + 1] == ',')
					{
						++count;
					}
					customProperty.erase(pos, count);
				}
            }
