...
//			System.out.println("encode--->" + datas.length);
			if (datas != null && datas.length > 0) {
				...
				if (gmPort.equals(localAddress.split(":")[1])) {
					...
					byte[] encryptDatas = null;
					if (eHeader.isFlagsCompressed()) {
						encryptDatas = Encrypt.encrypt(datas, new Random().nextInt());
					} else {
						encryptDatas = Encrypt.doXor(datas, new Random().nextInt());
					}
					int flags = eHeader.getFlags();
					int seqId = eHeader.getSendSequenceNumber();
					CRC32 crc = new CRC32();
					crc.update(encryptDatas);
					int crc32 = (int) crc.getValue();
					out.writeInt(encryptDatas.length + 12).writeInt(flags).writeInt(seqId).writeInt(crc32).writeBytes(encryptDatas);
					eHeader.setSendSequenceNumber(seqId + EncryptHeader.SEQUENCE_NUMBER_INCREMENT);
				}
				ctx.flush();
			}
		} catch (Exception e) {
