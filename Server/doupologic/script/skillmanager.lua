...
SkillManager[0027] =
	{
		...
		reduceFunc = function(lv,isMana,probability) --必须，lv：技能等级；isMana：主动技能是否法术
				local thisIsMana = true --true:法术减伤;false:物理减伤
				local thisProbability = 0.5
				local thisNumber = 0.3     --减伤自然数 ps.改为概率 --todo 需要物理法术分开？不需区分minitom
				return (thisIsMana == isMana and thisProbability > probability) and thisNumber or 0
			end,
	}
-----------------------------------------------------------------------------法攻降攻击
SkillManager[0028] =
...
SkillManager[0105] =
	{
		name = "地灵束缚", --琥嘉
		desc = function (lv)
				return "对单个敌人造成105%法术伤害，第1回合释放，冷却时间1回合" 			
			end,
		type = SkillManager_TYPE_ACTIVE_ROUND + 1, 
		start = 1, 
		isMana = true, 
...
SkillManager[0126] = 
	{
		...
		reduceFunc = function(lv,isMana,probability) --必须，lv：技能等级；isMana：主动技能是否法术
				...
				local thisProbability = 1
				local thisNumber = 0.4     --减伤自然数 ps.改为概率 --todo 需要物理法术分开？不需区分minitom
				return (thisIsMana == isMana and thisProbability > probability) and thisNumber or 0
			end,
	}
...
SkillManager[0137] =
	{
		name = "锋芒影刺", --若琳导师
		desc = function (lv)
				return "对前排敌人造成100%法术伤害，30%概率使对方晕眩（失去1个回合行动力）第1回合释放，冷却时间1回合" 			
			end,
		type = SkillManager_TYPE_ACTIVE_ROUND + 1, 
		start = 1, 
		isMana = true, 
...
SkillManager[0155] =
	{
		...
		addFunc = function(lv,myDeath,enemyDeath) --必须，lv：技能等级；myDeath：主动方的死亡人数；enemyDeath：被动方的死亡人数
				return myDeath * 0.2 --人数*每单位的伤害值
			end,
	}
...
SkillManager[0240] =
	{
		...
		reduceFunc = function(lv,isMana,probability) --必须，lv：技能等级；isMana：主动技能是否法术
				local thisIsMana = true --true:法术减伤;false:物理减伤
				local thisProbability = 0.8
				local thisNumber = 0.5     --减伤自然数 ps.改为概率 --todo 需要物理法术分开？不需区分minitom
				return (thisIsMana == isMana and thisProbability > probability) and thisNumber or 0
			end,
	}
SkillManager[0241] =
...
SkillManager[0280] =
	{
		...
		desc = function (lv)
				return "对单个敌人造成150%物理攻击伤害，第1回合释放，冷却时间1回合" 			
			end,
		type = SkillManager_TYPE_ACTIVE_ROUND + 1, 
		start = 1, 
		isMana = false, 
...
SkillManager[0302] =
	{
		name = "铁山拳", --苏千 吴昊
		desc = function (lv)
				return "对敌方单体造成280%物理伤害，无视免疫和伤害减免效果，第2回合释放，冷却时间1回合" 			
			end,
		type = SkillManager_TYPE_ACTIVE_ROUND + 1, 
		start = 2, 
		isMana = false, 
...
SkillManager[1202] =
	...
	SkillManager[1600] =
	{
		name = "风灵分形剑", --魂风
		desc = function (lv)
				return "对单个敌人造成100%法术伤害，100%概率使对方虚弱（减少50%攻击力，持续2次），第1回合释放，冷却时间1回合" 			
			end,
		type = SkillManager_TYPE_ACTIVE_ROUND + 0, 
		start = 1, 
		isMana = true, 
...
SkillManager[1605] =
		...
		SkillManager[1623] =
	{
		name = "赤血丹心", --玄衣
		desc = function (lv)
				return "为己方全体英雄增加持续回血状态（不受海心焰效果影响，持续2次），每回合释放（可以对处于不可以被治疗状态下的英雄生效）" 			
			end,
		type = SkillManager_TYPE_ACTIVE_ROUND + 0, 
		start = 1, 
		isMana = true, 
