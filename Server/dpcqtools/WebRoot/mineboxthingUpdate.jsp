...
<%
	...
	String rewards = request.getParameter("thing");
...
<script type="text/javascript">
	...
	function update() {
		...
		var url;
		url = "activities!updateMineBoxThingList.action";
		$.get(url, {
			id : id,
			chance : chance,
			thingCode : thingCode,
			...
			alert(dataObj);
			if (dataObj == '操作成功') {
				$("#right_content").load("mineboxthing.jsp",function(){$("#right_content").fadeIn(100);});
			}
		});
	}
...
<form id='from1' name='from1' onload="">
	...
	<dl>
		...
		<dd>
			<input id='type' value="<%=type%>" type="text" name='chance' readonly>
		</dd>
	</dl>
	<dl>
		<dt>
		...
		<dd>
			<input id='chance' value="<%=chance%>" type="text" name='chance'>
		</dd>
	</dl>
	<dl>
		<dt>
