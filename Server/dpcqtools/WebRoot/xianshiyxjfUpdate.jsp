...
<%
	...
	String saveJifenNum = request.getParameter("saveJifenNum");
...
<script type="text/javascript">
	...
	function update() {
		var id = $("#id").val();
		var saveJifenNum = $("#saveJifenNum").val();
		var rewards = $("#rewards").val();
		var thingCode = $("#thingCode").val();
		var url;
		var message;
		url = "activities!updateLimitHeroJifenList.action";
		$.get(url, {
			id : id,
			thingCode : thingCode,
			rewards : rewards
			...
			alert(dataObj);
			if (dataObj == '操作成功') {
				$("#right_content").load("xianshiyxjf.jsp",function(){$("#right_content").fadeIn(100);});
			}
		});
	}
...
<form id='from1' name='from1' onload="">
	...
	<dl>
		<dt>
			<label for="email">累计积分数</label>
		</dt>
		<dd>
			<input id='saveJifenNum' value="<%=saveJifenNum%>" type="text" name='saveJifenNum' readonly>
		</dd>
	</dl>
	<dl>
		<dt>
			<label for="email">积分对应奖励(可多个)</label>
		</dt>
		<dd>
