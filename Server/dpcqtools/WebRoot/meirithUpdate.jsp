...
<script type="text/javascript">
	...
	function update() {
		var id = $("#id").val();
		var thingsName = $("#thingsName").val();
		var price = $("#price").val();
		var things = $("#things").val();
		var thingCode = $("#thingCode").val();
		var url;
		var message;
		url = "activities!updateDailyDealsList.action";
		$.get(url, {
			id : id,
			...
			alert(dataObj);
			if (dataObj == '操作成功') {
				$("#right_content").load("meirith.jsp",function(){$("#right_content").fadeIn(100);});
			}
		});
	}
...
<form id='from1' name='from1' onload="">
	...
	<dl>
		<dt>
			<label for="email">天</label>
		</dt>
		<dd>
			<input id='day' value="<%=day%>" type="text" name='day' readonly>
		</dd>
	</dl>
	
	<dl>
		<dt>
			<label for="email">礼包名称</label>
		</dt>
		<dd>
			<input id='thingsName' value="<%=thingsName%>" type="text" name='thingsName'>
		</dd>
	</dl>
	
	<dl>
		<dt>
			<label for="email">礼包物品(固定6个)</label>
		</dt>
		<dd>
	...
	<dl>
		<dt>
			<label for="email">物品价格(固定6个)</label>
		</dt>
		<dd>
			<input id='price' value="<%=price%>" type="text" name='price'>
		</dd>
	</dl>
