...
<%
	...
	String rewardTimePoint = request.getParameter("rewardTimePoint");
...
<script type="text/javascript">
	...
	function update() {
		var id = $("#id").val();
		var rewardTimePoint = $("#rewardTimePoint").val();
		var rank = $("#rank").val();
		var rewards = $("#rewards").val();
		var thingCode = $("#thingCode").val();
		var url;
		var message;
		url = "activities!updateStrogerHeroList.action";
		$.get(url, {
			id : id,
			thingCode : thingCode,
			rewards : rewards
			...
			alert(dataObj);
			if (dataObj == '操作成功') {
				$("#right_content").load("dianfengqz.jsp",function(){$("#right_content").fadeIn(100);});
			}
		});
	}
...
<form id='from1' name='from1' onload="">
	...
	<dl>
		<dt>
			<label for="email">发奖时间段:</label>
		</dt>
		<dd>
			<input id='rewardTimePoint' value="<%= rewardTimePoint%>" type="text" name='rewardTimePoint' readonly>
		</dd>
	</dl>
	<dl>
		<dt>
			<label for="email">排名</label>
		</dt>
		<dd>
			<input id='rank' value="<%= rank%>" name='rank' type="text" readonly>
		</dd>
	</dl>
	
	<dl>
		<dt>
			<label for="email">奖励物品(单个)</label>
		</dt>
		<dd>
