...
<script type="text/javascript">
	...
	function showfrozen() {
		$("#tbody").empty();
	 	...
	 	if(serverId>0){
		  	$.getJSON("player!showShutup.action", {serverid:serverId}, function (data){
			  	if(data == null){
					$("#tbody").empty();
					alert('服务器连接失败！');
				}else{
				 	...
				 	$.each(dataObj, function(i,item) {
				 		var url = "&quot;player!shutupFree.action?id="+item.id+"&serverid="+serverId+"&quot;";
				  		$("#tbody").append(
								"<tr>"+
	        					"<td></td>"+
	...
	function frozen() {
		...
		$.get("player!shutup.action", {serverid:serverId ,name:name}, function (data){
			alert('禁言成功');
			$("#right_content").load("jjzh.jsp",function(){$("#right_content").fadeIn(100);});
		});
...
<h4>
	...
	&nbsp;&nbsp;&nbsp;
	<button id="frozen" onclick="frozen()">禁言账号</button>
</h4>
...
<table id="rounded-corner" style="width: 780xp">
	<thead>
		<tr>
			...
			<th scope="col" class="rounded">禁言时间</th>
			<th scope="col" class="rounded-q4">解除禁言</th>
		</tr>
	</thead>
	<tfoot>
		<tr>
