...
import mmo.tools.thread.CThread;
	...
	public final void run(long currTime) {
		...
		try {
			...
			while ((runEntity = eventQueue.poll()) != null) {
				...
				if (runntime > 1000) {
					LoggerError.error(Thread.currentThread() + "超长时间运行[" + (runEntity != null ? runEntity.getClass().getName() : runEntity)
					        + "], time=" + runntime);
				}
				running = null;
			...
			if (exeTime > 1000 * 30) {
				LoggerError.error(Thread.currentThread() + "执行时间超长：" + exeTime);
			}
		} catch (Exception e) {
			LoggerError.error(Thread.currentThread() + "事件抛出异常" + runEntity, e);
		}
	}
	...
	private final void execute(long currTime) {
		...
		if (rSize > 0) {
			...
			for (int ri = 0; ri < rSize; ri++) {
				...
				runntime = System.currentTimeMillis() - startTime;
				if (runntime > 1000) {
					LoggerError.error(Thread.currentThread() + "超长时间运行[" + running.getClass().getName() + "], time=" + runntime);
				}
				running = null;
			}
		}
