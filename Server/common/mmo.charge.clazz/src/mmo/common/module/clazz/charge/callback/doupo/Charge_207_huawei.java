...
public class Charge_207_huawei extends AChargeContextHandle {
	...
	protected HttpResponseMessage checkParameters(HttpRequestMessage request) {
		try {
			...
			if (!doCheck(sb.toString(), request.getParameter("sign"), ProjectCofigs.getParameter("huawei_payPubKey"))) {
				JSONObject json = new JSONObject();
				json.put("result", 1);
				return sendToClient(json.toString());
			}
		} catch (Exception e) {
			LoggerError.error("生成华为签名时异常", e);
			JSONObject json = new JSONObject();
			json.put("result", 1);
			return sendToClient(json.toString());
		}
	...
	public HttpResponseMessage callback(IoSession session, HttpRequestMessage request) {
		try {
			...
			LoggerError.error("huawei message =" + message);
			if (message != null) {
				return message;
			}
			String customOrder = request.getParameter("extReserved");
			int cents = (int) (Float.parseFloat(request.getParameter("amount")) * 100);
			byte chargeType = ChargeType.TYPE_ROLE_CHARGE_FAIL;
			String channelOrder = request.getParameter("orderId");
			String channel = "huawei";
			String channelProxy = "huawei";
			long proxyTime = 0;
			String userId = null;

			if ("0".equals(request.getParameter("result"))) {
			...
			LoggerError.error("处理华为充值通知异常", e);
		}
		return sendToClient("FAILURE");
	}
