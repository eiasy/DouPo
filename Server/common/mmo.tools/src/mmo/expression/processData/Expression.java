...
public class Expression {
	...
	public static int Calculate(String str) {
		...
		try {
			...
			while (!st.isEmpty()) {
				...
				if (isOperator(sign)) {
					String data1 = null;
					switch (getOperatorType(sign)) {
						case OPERATOR_TYPE_UNARY:
							int d2 = getValue(tSt.pop());
							result = mathMethod(d2, sign);
							break;
						case OPERATOR_TYPE_BINARY:
							d2 = getValue(tSt.pop());
							data1 = tSt.pop();
							int d1 = getValue(data1);
							result = mathMethod(d1, d2, sign);
							break;

					}
					tSt.push(String.valueOf(result));
				} else {
					tSt.push(sign);
				}
			}
	 ...
	 * 一元 运算符
	 */
	private final static byte OPERATOR_TYPE_UNARY  = 1;

	/**
	 * 二元运算符
	 */
	private final static byte OPERATOR_TYPE_BINARY = 2;
	...
	public static int[] getPostfixString_intArray(String infixStr) {
		...
		try {
			...
			while (!ts.isEmpty()) {
				temp = ts.pop();
				if (!isNumString(temp) && !isOperator(temp)) {
					isUnknown = true;
				}
				nts.push(temp);
			}
	...
	private static int mathMethod(int num, String operator) {
		if (equals(operator, "!")) {
