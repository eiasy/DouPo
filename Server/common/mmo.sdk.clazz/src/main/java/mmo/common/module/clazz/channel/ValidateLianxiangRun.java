...
import mmo.common.module.sdk.util.XmlUtil;
...
public class ValidateLianxiangRun extends HandRequestRun {
	...
	public void run() {
		try {
			...
			String lpsust = URLDecoder.decode(request.getParameter("token"), "utf-8");

			String url = ProjectCofigs.getParameter("lianxiang_url");

			// 组织参数
			StringBuilder paramBuilder = new StringBuilder();
			paramBuilder.append("lpsust=").append(lpsust).append("&").append("realm=").append(ProjectCofigs.getParameter("lianxiang_appId"));
			url = url + "?" + paramBuilder.toString();
			String result = HttpsUtil.request(url, "");

			if (result != null && !result.equals("")) {
				if (!XmlUtil.getElementValue(result, "AccountID").equals("")) {
					String id = XmlUtil.getElementValue(result, "AccountID");
					...
					message.put(HttpCData.A20001.username, XmlUtil.getElementValue(result, "Username"));
