 ...
 * 
 */
public class SheetSystemRepair extends AParseSheet {
	private static final String COL_CONTENT = "公告内容";
	private NoticeManager       noticeManager;

	public SheetSystemRepair(String sourceFile, HSSFSheet sheet, NoticeManager noticeManager) {
	...
	public boolean parse() {
		...
		for (int ri = 1; ri <= rowCount; ri++) {
			row = getRow(ri);
			if (row != null) {
				noticeManager.setNoticeSystemRepair(getStringValue(row, COL_CONTENT));
			}
		}
		return true;
	}
